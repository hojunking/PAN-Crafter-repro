...
그 뒤 **`recipe_lock`(QRC24_LOCK_V1_20260917)** 으로 공통 설정 C* 하나를 동결하고 사전 지정 **seed 20 개**(s1 41001/41006/41011/41016 · s2 41002… · 서버마다 4 개) 를 돈다. **lock 파일이 없으면 seed 단계 config 자체가 생성되지 않는다**(생성기가 SystemExit). 동결 규칙은 n_joint → n_H → med_E_H → 동률이면 G23 유지이고, β .15·rA .02 같은 제3 후보를 넣지 않는다. 도구 `tools/qrc24_lock.py --status/--decide/--write --profile/--verify`.
