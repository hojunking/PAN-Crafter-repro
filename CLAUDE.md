...
config `PAKD50_<case>_W112_D123_WV3_T0_S<seed>_FRESH50_v1`(`tools/gen_pakd50_configs.py`), 큐 `config/queues/pakd50_<srv>_stage{1,2}.txt`(stage 1 J0→F0→JR→FR; stage 2 JQ→FQ→XJ 는 s1 의 J0-1234 뒤 gate `pakd50` 가 λE 를 고정하고 연다), τR/λE 는 `tools/pakd50_calibrate.py`(고정값 `assets/pakd50/calibration_resolved.json`), gate `tools/pakd50_unit_tests.py`, 기동 `./tools/pakd50_prepare.sh`. 후보 격자는 `GRID1010_50K_v1`(eval_epoch 5, 50 후보 전부 보존). C1/C2(routing P/JK0/JE0, D/LF, TCOPY/CONT) 는 미구현.
