"""G20 provenance and immutable artifact helpers, independent of live WV3."""
...
def camp(root, server):
    if server not in {'s1', 's2', 's3', 's4', 's5'}:
        raise ValueError('G20 local server must be s1..s5')
    return Path(root) / 'work_dir/_g20' / server
...
def source_identity(root=ROOT):
    ...
    from qg40.common import source_identity as legacy_identity
    root = Path(root)
    result = legacy_identity(root)
    paths = sorted((root / 'g20').glob('*.py'))
    from g20.plan import SOURCE_SHAS
    paths += [root / name for name in SOURCE_SHAS]
    paths += sorted((root / 'tools').glob('g20_*.py'))
    paths += sorted((root / 'tools/metrics').glob('*.py'))
    paths += [root / 'tools/repair_qb_ms.py'] if (root / 'tools/repair_qb_ms.py').is_file() else []
    paths += [root / name for name in ('g20/sensor_sources.json', 'g20/SENSOR_SOURCE_EVIDENCE.md')
...
def numerical_compatibility(origin, consumer):
    ...
    if differences:
        raise ValueError('G20 reference numerical/runtime mismatch: ' + ', '.join(differences))
...
def load_checkpoint_model(cfg, directory, device='cpu', expected_source=None):
    ...
    if identity.get('model_sha256') != sha256(weights) or identity.get('config_sha256') != object_sha(cfg):
        raise ValueError('G20 checkpoint bytes/config mismatch')
    if expected_source is not None and identity.get('source_identity') != expected_source:
        raise ValueError('G20 checkpoint execution release mismatch')
    field, args = cfg['g20'], cfg['model_args']
