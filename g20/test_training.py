...
from fh12.calibration import CONSTANT_ALIGNER_Q
from g20.calibration import (axis16_q, compute_calibration,
                              select_calibration_indices, calibrate)
from g20.common import atomic_json, object_sha, sha256
from g20.losses import routed_student_backward, student_losses, teacher_loss
from g20.model import build_model, state_hash, sync_frontend
from g20.plan import CASES, CAMPAIGN_ID, GRID_STEPS, build_config
from g20.training import (BatchStream, aligner_factor, cosine_factor, make_scheduler,
...
class G20TrainingTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_student_routing_detached_teacher_and_exact_one_axis_profiles(self):
        ...
        for profile in ("A1", "A9", "K05", "K20", "E1", "E4"):
            ...
            self.assertTrue(torch.equal(changed["soft"], losses["soft"] * (2 if profile == "K20" else .5 if profile == "K05" else 1)))
            self.assertTrue(torch.equal(changed["edge_weighted"], losses["edge_weighted"] * (.5 if profile == "E1" else 2 if profile == "E4" else 1)))
    ...
    def test_teacher_odd_only_consistency_rng_c3_and_no_u_synthetic(self):
        ...
        low = teacher_loss(model, out, gt, pan, ms, 1, torch.Generator().manual_seed(4), 3e-5)
        self.assertEqual(low['offset_weight'], 3e-5)
        self.assertTrue(torch.equal(low['epsilon'], odd['epsilon']))
        self.assertTrue(torch.equal(low['off'], odd['off']))
    ...
    def test_schedule_boundary_and_grid(self):
        ...
        for step in (0, 1, 99, 100, 10000, 24239):
            self.assertEqual(aligner_factor(step, "A1"), 1.)
        self.assertEqual(aligner_factor(24240, "A1"), 1.)
        self.assertEqual(aligner_factor(50000, "A1"), 1.)
        self.assertEqual(cosine_factor(0), 0.)
        self.assertEqual(cosine_factor(100), 1.)
        self.assertEqual(cosine_factor(50000), 0.)
        for profile, expected in (('BASE', 3e-6), ('A1', 1e-6), ('A9', 9e-6)):
            from g20.plan import PROFILES
            params = [nn.Parameter(torch.ones(1)), nn.Parameter(torch.ones(1))]
            optimizer = torch.optim.AdamW([dict(params=[params[0]], name='U', lr=1e-4),
                dict(params=[params[1]], name='A', lr=PROFILES[profile].student_a_peak_lr)])
            scheduler = make_scheduler(optimizer, profile)
            for step in (100, 24240, 40000):
                self.assertAlmostEqual(scheduler.lr_lambdas[1](step) * expected, cosine_factor(step) * expected)

    def test_fixed_diagnostics_routes_gradients_and_restores_model_rng(self):
        from g20.diagnostics import capture_diagnostics, diagnostic_indices, gradient_statistics
    ...
    def test_sampler_optimizer_scheduler_rng_exact_resume(self):
        def setup():
            ...
            return params, opt, make_scheduler(opt, "A1"), BatchStream(13, 4, 9), torch.Generator().manual_seed(55)
        def advance(parts):
    ...
    def test_registered_recipe_rejects_changed_method_and_resume_bundle(self):
        ...
        cfg["g20"]["dataset_manifest"] = "/bound/data.json"
        self.assertEqual(validate_config(cfg), CASES[0])
        cfg["g20"]["consistency_weight"] *= 3
        with self.assertRaises(ValueError):
            validate_config(cfg)
    ...
    def test_common_window_exact20h_and_no_new_block_after16h(self):
        cfg = build_config(CASES[0])
        now = dt.datetime.now(dt.timezone.utc)
        with tempfile.TemporaryDirectory() as tmp:
            window = Path(tmp) / cfg["g20"]["window_path"]
            atomic_json(window, dict(campaign_id=CAMPAIGN_ID, t0_utc=(now - dt.timedelta(hours=17)).isoformat(),
    ...
    def test_actual_trainer_deadline_state_and_resume_matches_uninterrupted(self):
        ...
        def initialize(root, case):
            ...
            cfg["g20"]["dataset_manifest"] = "data.json"
            if case.role == 'S':
                cfg['g20'].update(tau_R=.2, q_ref=.4, teacher_sha256='t' * 64, q_cache_sha256='q' * 64)
            (Path(root) / "config.yaml").write_text(yaml.safe_dump(cfg))
            atomic_json(Path(root) / "data.json", dict(sensor="GF2", num_bands=4, max_pixel=1023))
            start = dt.datetime.now(dt.timezone.utc) - dt.timedelta(hours=1)
            atomic_json(Path(root) / cfg["g20"]["window_path"], dict(campaign_id=CAMPAIGN_ID,
                t0_utc=start.isoformat(), deadline_utc=(start + dt.timedelta(hours=20)).isoformat()))
            return cfg
        ...
        def run(root, updates, case, resume=False):
            ...
            def reference(*args, **kwargs):
                with torch.random.fork_rng(devices=[]):
                    ...
                    teacher = Model()
                return teacher, dict(tau_R=.2, q_ref=.4, teacher_checkpoint_sha256='t' * 64,
                                     q_cache_sha256='q' * 64, teacher_run_id=case.teacher_run_id,
            ...
            logged = io.StringIO()
            with patch("g20.data.build_dataset", return_value=Dataset()), \
                    patch("g20.references.load_reference", side_effect=reference), \
                    patch("g20.evaluation.FRMetrics", return_value=object()), \
                    patch("g20.evaluation.evaluate_model", return_value=metric), \
                    patch("g20.training.GRID_STEPS", (1, 2, 3)), \
                    patch("g20.training.FULLSTATE_STEPS", (1, 2, 3)), \
                    patch("g20.diagnostics.capture_diagnostics", side_effect=TimeoutError("P1 budget exhausted")), \
                    ...
                    patch("g20.training.build_model", side_effect=lambda *a, **k: (Model(), {})), \
                    patch("g20.training.source_identity", return_value={"synthetic_test": True}), \
                    patch("g20.training.before_deadline", side_effect=deadline), \
                    patch("g20.training.DataLoader", side_effect=loader), \
        ...
        for case in (CASES[0], next(c for c in CASES if c.role == "S" and c.is_resolved)):
...
def numerical_smoke():
    suite = unittest.defaultTestLoader.loadTestsFromTestCase(G20TrainingTests)
