"""Fixed train128 distributions and train24 gradient evidence for G20.
...
from g20.calibration import axis16_q, select_calibration_indices
from g20.common import check_deadline, object_sha
from g20.losses import STUDENT_PROFILES, student_losses, teacher_loss
from g20.model import state_hash
from g20.training import restore_rng, rng_state
    ...
    try:
        ...
        with torch.no_grad():
            for start in range(0, len(fixed), batch_size):
                ...
                out_gx, out_gy = scharr(out['y'])
                append('output_highpass_energy', (.5 * (out_gx.abs() + out_gy.abs())).mean((1, 2, 3)))
                if teacher is not None:
                    tout = teacher(pan, ms, lp)
                    # The Student's initial A is a deep copy of this frozen A.
                    append('initial_delta', tout['delta'])
                    append('delta_minus_initial', out['delta'] - tout['delta'])
                    initial_border = border_sampling(tout['delta'], *pan.shape[-2:])
                    append('initial_border_sampling_fraction', initial_border['border_sampling_fraction'])
        ...
        for index in fixed[:n_gradient]:
            ...
            if teacher is not None:
                ...
                unweighted_soft = torch.autograd.grad(losses['soft'] / STUDENT_PROFILES[profile][1], params, retain_graph=True)
                unweighted_edge = torch.autograd.grad(losses['edge'], params, retain_graph=True)
                ...
                gradient_rows.append(dict(index=int(index), soft_hard=gradient_statistics(soft, hard),
                    soft_edge=gradient_statistics(soft, edge),
                    weighted_norms=dict(hard=_gradient_norm(hard), soft=_gradient_norm(soft), edge=_gradient_norm(edge)),
                    unweighted_norms=dict(hard=_gradient_norm(hard), soft=_gradient_norm(unweighted_soft), edge=_gradient_norm(unweighted_edge)),
                ...
                gradient_rows.append(dict(index=int(index), consistency_rec=gradient_statistics(con, rec),
                    consistency_probe='ON: zero-based odd update_index=1; separate fixed diagnostic RNG',
                    consistency_off_a_gradient_norm=_gradient_norm(rec),
                    unweighted_consistency_a_gradient_norm=_gradient_norm(con) / consistency_weight,
        ...
        summary = dict(schema="G20_DIAGNOSTICS_v1", status="MEASURED", synthetic_test=synthetic_test,
        ...
        if teacher is not None:
            ...
            summary.update(advantage_fraction=float(result["advantage_fraction"].mean()),
                initial_delta_difference_distribution=_distribution(np.linalg.norm(result['delta_minus_initial'], axis=1)),
                initial_border_sampling_fraction=_distribution(result['initial_border_sampling_fraction']),
                soft_edge_gradient_cosine=_median([row['soft_edge']['cosine'] for row in gradient_rows]),
