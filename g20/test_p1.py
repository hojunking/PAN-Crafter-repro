...
from g20.common import read_json
from qg40.exposure import record_batch, validate_counts, exposure_report
from g20.p1 import bounded_deadline, record_training_diagnostics
...
class P1Tests(unittest.TestCase):
    ...
    def invoke(self, capture, sizes):
        with patch('g20.diagnostics.capture_diagnostics', capture), \
    ...
    def test_receipt_io_error_does_not_stop_base_or_publish_evidence(self):
        ...
        sizes = Mock(return_value={'status': 'MEASURED'})
        before = torch.get_rng_state().clone()
        with patch('g20.p1.atomic_json', side_effect=PermissionError('diagnostics-only read-only')), \
                self.assertLogs('g20.p1', level='WARNING') as logs:
    ...
    def test_receipt_io_failure_cannot_hide_weight_mutation(self):
        def mutate(*args, **kwargs):
            with torch.no_grad():
                self.model.weight.add_(1)
            raise ValueError('analysis failure')
        with patch('g20.p1.atomic_json', side_effect=PermissionError('read-only')), \
                self.assertLogs('g20.p1', level='WARNING'), \
