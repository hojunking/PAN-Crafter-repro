"""Same-checkpoint official reporting for FT20K and fresh100K candidate budgets."""
...
from gfb20.common import (ROOT, atomic_json, check_deadline, object_sha, read_json,
    read_config, resolved_path, sha256, source_identity, utcnow)
from gfb20.plan import CAMPAIGN_ID, grid_steps, validate_config
from gfb20.evaluation import evaluate_model, FRMetrics
...
def summarize_grid(records, updates, context=None):
    """Pure selection; t=0 and diagnostic50K are never official candidates."""
    ...
    chosen = dict(EXACT_FINAL=by_step[updates],
        ...
        RAW_AUX=min(records, key=lambda r: (-r['fr']['hqnr'], -r['rr']['scc'],
                                           r['rr']['ergas'], r['update'])))
    selections = {label: dict(row, selection_id=label, primary=label != 'RAW_AUX',
    ...
    return dict(context or {}, schema='GFB20_OFFICIAL_SUMMARY_v1', campaign_id=CAMPAIGN_ID,
...
def process(config_path, root=ROOT, device='cuda', deadline=None):
    ...
    parent = read_json(resolved_path(cfg['gfb20']['parent_manifest'], root)) if case.is_ft else None
    ...
    context = dict(run_id=case.run_id, case_id=case.case_id, server=case.server, stage=case.stage,
        block_id=case.block_id, profile=cfg['gfb20']['profile'], config_sha256=object_sha(cfg),
        ...
        parent_compute=parent_compute, costs=costs,
