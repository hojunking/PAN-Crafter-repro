"""Dedicated GF2-B20 tabs only. Raw precision plus display formats and readback."""
import datetime as dt
import json
from pathlib import Path
from fh12.upload import a1,same_cell,legacy_constants
from reporting_extra.sensor_backfill import write_locks
from gfb20.common import (ROOT,read,read_json,read_config,run_dir,camp,object_sha,
                         source_identity,atomic_json,sha256,utcnow)
from gfb20.plan import CAMPAIGN_ID,SHEET_TABS,case_for,validate_config
...
def flatten_summary(summary,case,root=ROOT):
    ...
    values=dict(campaign_id=CAMPAIGN_ID,run_id=case.run_id,case_id=case.case_id,server=case.server,
        ...
        profile=summary.get('profile',case.profile),alpha=case.alpha_target,beta=case.beta_target,
        edge=case.lambda_edge_target,coefficient_ramp_updates=case.coefficient_ramp_updates,
        ...
        calibration_hours=0.,shared_calibration_hours=read(camp(root,case.server)/'status.json').get('mixed_calibration_hours',0.),
    ...
    if case.profile=='B_SELECTED':
        from gfb20.plan import PROFILES
        chosen=PROFILES[summary['profile']]
        values.update(alpha=chosen['alpha_target'],beta=chosen['beta_target'],edge=chosen['lambda_edge_target'])
    ...
    values['notes']='Main EXACT_FINAL; co-primary RR_VAL_SELECTED; RAW_AUX test-aware. Native RR20/FR20 DN1023, no masking. FT is weights-initialized, not fresh/exact parent-resume. JQM SRF-substitute.'
...
def row_values(run,root=ROOT):
    from gfb20.postrun import summarize_grid,_verify_grid,verify_endpoint_progress
    ...
    data=read_json(cfg['gfb20']['dataset_manifest'])
    ...
    if (summary.get('campaign_id')!=CAMPAIGN_ID or summary.get('run_id')!=case.run_id
        ...
        raise ValueError('GFB20 upload summary differs from complete official checkpoint grid')
    ...
    from gfb20.assets import _finite_state,_fullstate
...
def apply_upsert(ws,values):
    ...
    if ws.title!=SHEET_TABS.get(server) or values['campaign_id']!=CAMPAIGN_ID:
        raise ValueError('Only this B20 server dedicated tab may be written')
    header=list(values)
    current=ws.row_values(1)
    if current and current!=header:raise ValueError('Existing B20 tab has another schema; refusing header overwrite')
    ...
    if len(matches)>1:raise ValueError('Duplicate B20 run rows; do not overwrite ambiguous results')
    ...
    for i,key in enumerate(header,1):
        if key in decimal_labels or key.endswith('hours') or key.endswith('val ERGAS'):
    ...
    if any(not same_cell(actual[i] if i<len(actual) else '',values[key]) for i,key in enumerate(header)):
        raise ValueError('B20 raw-value readback differs')
    ...
    if ws.row_values(row,value_render_option='UNFORMATTED_VALUE')[status_col-1]!='READBACK_VERIFIED':
        raise ValueError('B20 upload status readback failed')
...
def upload_run(run,root=ROOT,*,activated=False,worksheet=None):
    if not activated:raise PermissionError('Explicit GFB20 activation required for live Sheet writes')
