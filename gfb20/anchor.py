...
from gfb20.common import ROOT, immutable_json, object_sha, read_config, read_json, resolved_path
from gfb20.plan import BUNDLE, CAMPAIGN_ID, PARENT_ASSETS, SOURCE_SHAS, case_for
...
def expected_anchor(case, parent, root=ROOT):
    ...
    if parent_id in PARENT_ASSETS['parents']:
        ...
        filename = BUNDLE + '/GFB20_ParentAssets.json'
        return reported['reported_metrics'], dict(kind='IMMUTABLE_AUTHOR_BUNDLE_FULL_PRECISION',
            path=filename, sha256=SOURCE_SHAS[filename], source_cell=reported['source_cell'],
            published_model_sha256=reported['published_model_sha256'],
            resolved_parent_model_sha256=parent['parent_model_sha256'])
    trunk_ids = {'P3NEW1': 'A11', 'P3NEW2': 'A14'}
    if parent_id not in trunk_ids or parent.get('parent_kind') != 'GFB20_FRESH_TRUNK':
        raise ValueError('Unregistered parent has no permissible anchor')
    trunk = case_for(trunk_ids[parent_id])
...
def verify_and_save_parent_anchor(case, parent, anchor, output_path, root=ROOT):
    ...
    try:
        ...
        anchor_sha = None
    result.update(schema='GFB20_PARENT_ANCHOR_COMPARISON_v1', campaign_id=CAMPAIGN_ID,
