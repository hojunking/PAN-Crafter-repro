...
from gfb20.diagnostics import (border_summary, gradient_probe, low_active_mass,
    parameter_snapshot, parameter_update_norm, preserve_runtime, rng_state)
from gfb20.losses import coefficients, routed_student_backward, student_losses
from gfb20.plan import build_config, case_for, grid_steps
from gfb20.stream import PairedBatchStream, gamma_ids
from gfb20.training import (cosine_factor, initialize_student, make_optimizer,
    make_scheduler, model_hashes, select_validation, val_diverged,
    verify_batch_metadata, verify_paired_initialization, pause_reason)
...
class LossTests(unittest.TestCase):
    ...
    def test_coefficients(self):
        self.assertEqual(coefficients('H010', 0)['alpha'], .1)
        for p, key, base, goal in [('K1', 'beta', .1, 1.), ('E100', 'lambda_edge', .002, .1)]:
            self.assertEqual(coefficients(p, 0)[key], base)
            self.assertAlmostEqual(coefficients(p, 500)[key], (base + goal) / 2)
            self.assertAlmostEqual(coefficients(p, 1000)[key], goal)
            self.assertAlmostEqual(coefficients(p, 100000)[key], goal)
        with self.assertRaises(ValueError): coefficients('B_SELECTED', 10)

    def test_detached_weights(self):
        loss = student_losses({'y': self.y}, {'y': self.target}, self.gt, .04, self.q, 'K1', 1000)
        loss['L_U'].backward()
    ...
    def test_actual_separate_routing(self):
        model = Tiny()
        for profile in ('BASE', 'K1', 'E100', 'H010'):
            ...
            if profile == 'BASE': base_a = actual_a
            if profile in ('K1', 'E100'):
                self.assertTrue(all(torch.equal(x, y) for x, y in zip(base_a, actual_a)))

    def test_gradient_and_border_diagnostics(self):
        model = Tiny()
        loss = student_losses(model(self.gt, self.gt[:, :1]), {'y': self.target}, self.gt, .04, self.q, 'E100', 1000)
...
class StreamTests(unittest.TestCase):
    ...
    def test_actual_native_and_mixed_metadata(self):
        ...
        row = torch.tensor([[source, view, 1, 1, 1, gamma] for source, view, gamma in stream.batch_at(0)])
        self.assertEqual(verify_batch_metadata(stream, row, 'NATIVE0'),
                         [list(r) for r in stream.batch_at(0)])
        row[:, 4] = row[:, 5]
        verify_batch_metadata(stream, row, 'PANMIX')
        row[0, 1] = (row[0, 1] + 1) % 4
        with self.assertRaises(ValueError): verify_batch_metadata(stream, row, 'PANMIX')

    def test_actual256_consume_and_json_resume(self):
        from gfb20.common import immutable_json, read_json
        stream = PairedBatchStream(1024, 48, 96501)
        expected, consumed = stream.initial_prefix(), []
        for _ in range(6):
            meta = torch.tensor([[s, v, 1, 1, 1, g] for s, v, g in stream.batch_at(stream.cursor)])
            actual = verify_batch_metadata(stream, meta, 'NATIVE0')
            consumed.extend(actual[:max(0, 256 - len(consumed))]); stream.advance()
        self.assertEqual(consumed, expected['triples'])
        with tempfile.TemporaryDirectory() as tmp:
            file = Path(tmp) / 'actual.json'
            immutable_json(file, dict(triples=consumed))
            self.assertEqual(read_json(file)['triples'], expected['triples'])
        ...
        self.assertEqual(resumed.initial_prefix()['triples'], consumed)


class TrainingTests(unittest.TestCase):
    def test_stop_now_control_only(self):
        from gfb20.common import atomic_json
        with tempfile.TemporaryDirectory() as tmp:
            file = Path(tmp) / 'control.json'
            with patch('gfb20.training.before_deadline', return_value=True):
    ...
    def test_fork_preserves_parent_a_not_teacher(self):
        ...
        with patch('gfb20.training.build_model', side_effect=AssertionError('No fresh construction allowed')):
    ...
    def test_exact_resume_optimizer_sampler_rng(self):
        ...
        def one():
            ...
            loss = student_losses(model(gt, gt[:, :1]), {'y': target[ids]}, gt, .02,
                                  torch.full((4,), .4), 'K1', stream.completed_updates)
            routed_student_backward(model, loss)
