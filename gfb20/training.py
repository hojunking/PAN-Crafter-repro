"""Isolated B20 Student training: parent U+A FT, paired streams and exact resume.
...
from gfb20.common import (ROOT, CAMPAIGN_ID, apply_runtime_policy, atomic_json,
...
from gfb20.diagnostics import (diagnostic_ids, low_active_mass, parameter_snapshot,
    parameter_update_norm, preserve_runtime, rng_state, run_diagnostics)
from gfb20.losses import routed_student_backward, student_losses
from gfb20.plan import diagnostic_steps, fullstate_steps, grid_steps, validate_config
...
def cosine_factor(completed_updates, warmup=100, total=20000):
    if (isinstance(completed_updates, bool) or int(completed_updates) != completed_updates
            or not 0 <= completed_updates <= total or total not in (20000, 100000) or warmup != 100):
        raise ValueError('B20 requires warmup100 and its exact20K/100K horizon')
...
def make_scheduler(optimizer, warmup=100, total=20000):
    if [group.get('name') for group in optimizer.param_groups] != ['U', 'A']:
        raise ValueError('B20 scheduler requires independent explicit U/A groups')
...
def make_optimizer(model, cfg):
    optimizer = torch.optim.AdamW([
        ...
        dict(params=model.aligner.parameters(), lr=cfg['gfb20']['a_peak_lr'], name='A')],
...
def runtime_context(cfg, root=ROOT, deadline_arg=None):
    window = read_json(resolved_path(cfg['gfb20']['window_path'], root))
    ...
    if (window.get('campaign_id') != CAMPAIGN_ID or start.tzinfo is None or end.tzinfo is None
            or end - start != dt.timedelta(hours=20)):
        raise ValueError('B20 requires its shared immutable t0+20h window')
    if deadline_arg and deadline_arg != window['deadline_utc']:
        raise ValueError('B20 deadline cannot be overridden/restarted')
    if dt.datetime.now(dt.timezone.utc) < start:
        raise ValueError('B20 window has not started')
    return window, (start + dt.timedelta(hours=18)).isoformat(), window['deadline_utc']
...
def verify_batch_metadata(stream, meta, profile):
    ...
    if meta.shape != (stream.batch_size, 6) or meta.dtype != torch.int64:
        raise ValueError('B20 source/view/gamma metadata shape/dtype differs')
    expected = torch.tensor(stream.batch_at(stream.cursor), dtype=torch.int64)
    actual = meta[:, [0, 1, 5]].cpu()
    if (not torch.equal(actual, expected) or not bool((meta[:, 2:4] == 1).all())
            or not torch.equal(meta[:, 4].cpu(), expected[:, 2] if profile == 'PANMIX'
                               else torch.ones(stream.batch_size, dtype=torch.int64))):
        raise ValueError('B20 source/view/gamma draw or native-input identity differs')
    return actual.tolist()


def verify_paired_initialization(case, hashes, prefix, root=ROOT):
    from gfb20.plan import CASES
    checked = []
    for peer in CASES:
        if (peer.run_id == case.run_id or peer.server != case.server
                ...
                or peer.model_seed != case.model_seed or peer.init_mode != case.init_mode):
            continue
        folder = Path(root) / 'work_dir' / peer.run_id
        previous = read(folder / 'init_manifest.json')
        if not previous:
            continue
        if previous['hashes'] != hashes or read(folder / 'diagnostics/paired_stream_first256.json') != prefix:
            raise ValueError('Matched B20 arms differ at t=0 U/A/full tensors or sample/view/gamma prefix')
        checked.append(peer.case_id)
    return checked


def _resolved_profile_assets(cfg, assets, native_train, root):
    """Only scales and cache change for mixcal; frozen Teacher stays identical."""
    from gfb20.data import GFB20Dataset
    field, native = cfg['gfb20'], assets['reference']
    reference, q, mixed = dict(native), assets['q'], None
    if field['profile'] in ('NAT_MIXCAL', 'PANMIX'):
        from gfb20.calibration import load_mixed_reference
        mixed, q = load_mixed_reference(resolved_path(field['mixed_calibration_manifest'], root),
                                        native_reference=native, dataset=native_train)
    ...
    dataset = GFB20Dataset(native_train, augmentation_manifest=augmentation, profile=field['profile'])
    return dataset, reference, q, mixed


def train_run(config_path, device='cuda', resume=False, deadline_arg=None, root=ROOT):
    from gfb20.assets import load_training_assets
    from gfb20.controller import authorize_train
    root, config_path = Path(root).resolve(), Path(config_path)
    cfg = read_config(config_path)
    case = validate_config(cfg, require_bound=True)
    field, profile, total = cfg['gfb20'], cfg['gfb20']['profile'], case.updates
    policy = apply_runtime_policy(root)
    if field['runtime_policy_sha256'] != policy['sha256']:
        raise ValueError('Bound B20 runtime policy differs')
    ...
    if dev.type != 'cuda' or not torch.cuda.is_available():
        raise RuntimeError('Registered B20 training requires CUDA; no CPU fallback')
    ...
    if (data.get('schema') != 'G20_DATA_v1' or data.get('sensor') != 'GF2'
        ...
        raise ValueError('B20 requires unchanged GF2/C4/DN1023 data')
    ...
    train_dataset, reference, q, mixed = _resolved_profile_assets(cfg, assets, datasets['train'], root)
    q = torch.as_tensor(q, dtype=torch.float32, device=dev)
    expected_q_shape = (datasets['train'].base_count, 4) + ((3,) if mixed else ())
    if tuple(q.shape) != expected_q_shape or not bool(torch.isfinite(q).all()) or bool((q < 0).any()):
        raise ValueError('B20 full-training sample/view/gamma q cache differs')
    ...
    if resume:
        ...
        if last.get('training_state_sha256') != sha256(wd / 'last/training_state.pt'):
            raise ValueError('B20 resume checksum mismatch')
        ...
        if (not state.get('full_state') or state.get('precision') != 'fp32'
            ...
            raise ValueError('B20 exact-resume model/config/source/assets differ')
        start = read_json(wd / 'meta/training_start_manifest.json')
        if any(start.get(k) != v for k, v in identity_context.items()):
            raise ValueError('B20 resume start provenance differs')
        ...
        if not 0 <= update <= total or scheduler.last_epoch != update or stream.completed_updates != update:
            raise ValueError('B20 resume optimizer/scheduler/sampler update mismatch')
        if (len(consumed_prefix) != min(update * cfg['batch_size'], 256)
                or consumed_prefix != prefix['triples'][:len(consumed_prefix)]):
            raise ValueError('B20 actual consumed prefix differs on resume')
        ...
        immutable_json(wd / 'meta/training_start_manifest.json', dict(campaign_id=CAMPAIGN_ID,
            ...
            parent_step=case.parent_step or 0, init_mode=case.init_mode,
    ...
    def save_candidate(folder, include_state):
        ...
        if folder.exists():
            ...
            if (any(value.get(k) != v for k, v in expected.items())
                ...
                raise ValueError('Immutable B20 candidate changed')
            return value
    ...
    def diagnose():
        ...
        value = run_diagnostics(model, teacher, train_dataset, q, reference, profile, update,
            ids, str(dev), parent_delta=baseline, heavy=update in (0, 5000, total), deadline=final_deadline)
    ...
    def boundary():
        ...
        if not any(row['update'] == update for row in records):
            ...
            eval_seconds += time.monotonic() - started
            print(f'[GFB20 {case.case_id} {update}/{total}] HQNR={result["fr"]["hqnr"]:.8f} '
        ...
        if state_hash(teacher.state_dict()) != teacher_hash:
            raise ValueError('B20 frozen Teacher mutated')
    ...
    try:
        ...
        if case.is_ft:
            from gfb20.anchor import verify_and_save_parent_anchor
        ...
        while update < total:
            ...
            for gt, _lms, ms, lp, pan, meta in loader:
                ...
                if not bool(torch.isfinite(losses['L_U']) & torch.isfinite(losses['L_A'])):
                    raise FloatingPointError('Nonfinite B20 fitting loss')
                ...
                # The forward/backward can itself cross 18h; never optimizer.step afterwards.
                reason = pause_reason(pause[0], optimizer_deadline, control_path)
                if reason:
                    save_resume(); status(reason); return 75
                ...
                if len(consumed_prefix) == 256:
                    if consumed_prefix != prefix['triples']:
                    ...
                    if not actual_path.exists():
                        immutable_json(actual_path, dict(expected_prefix_sha256=object_sha(prefix),
                            triples=consumed_prefix, count=256, actual_loader_verified=True))
                ...
                if update % cfg['log_iter'] == 0:
                    print(f'[GFB20 {case.case_id}/{profile}] local={update}/{total} '
...
def recover_exact_endpoint(root, wd):
    ...
    if (identity.get('update') != case.updates or not identity.get('full_state')
        ...
        raise ValueError('B20 endpoint recovery identity differs')
    ...
    for key in ('config_sha256', 'source_identity', 'data_sha256', 'reference_sha256',
        ...
        if state.get(key) != identity.get(key) or start.get(key) != identity.get(key):
            raise ValueError('B20 endpoint provenance changed')
    if (not state.get('full_state') or state.get('update') != case.updates
        ...
        raise ValueError('B20 endpoint fullstate/model/scheduler differs')
    data = read_json(resolved_path(cfg['gfb20']['dataset_manifest'], root))
    if object_sha(data) != identity['data_sha256']:
        raise ValueError('B20 endpoint dataset identity changed')
    stream = PairedBatchStream(data['splits']['train']['count'], cfg['batch_size'], case.stream_seed)
    stream.load_state_dict(state['sampler'])
    if stream.completed_updates != case.updates:
        raise ValueError('B20 endpoint sampler differs')
