# S3 최종 실험 계획 — NA104 전체 보류 case 포함

**작성일:** 2026-09-12 · **버전:** FINAL-v1  
**서버:** s3 · **Backbone:** W104·D122 · **입력:** WV3 9ch · **Aligner:** 없음  
...
> 이 파일은 s3에 단독 전달할 수 있도록 해당 서버의 전체 case·순서·대조와 공통 계약을 포함한다. 원 ID와 완료 결과는 보존한다. 기존 보류군의 첫 탐색은 전부 편성하며, 후보 선발은 추가 확장·최종 채택에 적용한다. 모든 조합이 효과적이라는 가정은 하지 않는다.

## S3-0. 전달 결론

**s3는 s2 우승 후보의 교차 검증, IV/GC/SC 전 통계 모드, 표현별 방향/성분 선택, 복합 통계와 window·변환·domain 확장을 담당한다.**

가장 먼저 **Q09(R3+GV-FIX), Q12(R3+EDGE-H), Q08(R3+GV-T)**를 완료해 s2에서만 관측된 후보/대조의 빈칸을 채운다. s3에서 R1이 좋고 R2가 나쁜 결과가 이미 있으므로, s2의 순위를 그대로 가져오지 않는다. X01–X08의 직접 대조로 Teacher-error hard와 구조 감독의 결합을 함께 확인한다.

**원래 보류 39개를 전부 P3–P5에 넣었다.** GC를 먼저 읽되 IV·SC의 다섯 모드도 빠뜨리지 않는다. GV용 MASS/SHUF 결과로 GC·SC의 방향 효과를 대신 설명하지 않도록 X09–X12의 표현별 대조를 추가했다.

## S3-1. 실행 순서 요약

1. **P0:** 완료 결과·hash·공통 HQNR 평가·fitting/gradient 산출물을 확인한다.
2. **P1:** Q09→Q12→Q08. 기존 Q06/Q07/Q10과 함께 같은 서버의 GV 모드 사다리를 완성한다.
3. **P2:** X01–X06·X08, CTLHSCALE·X07로 구조 결합과 hard/soft의 역할을 분리한다.
4. **R1 반복:** core10을 Student seed777에서 대응 실행한다.
5. **P3:** GC의 H/FIX/WH/AD/T를 우선, 이어 IV·SC의 다섯 모드를 전부 수행한다.
6. **P4:** A/B 기준, GV 및 GC/SC의 MASS/SHUF·SIGN/CAP/COMPADV, A+B 2×2, GV+SC 및 half-scale 대조.
7. **R2 반복:** 같은 core10을 Student seed2026에서 대응 실행한다.
8. **P5:** 보류된 window3/7/357, M2, STD, LOG, residual domain을 모두 수행한다.
9. **원 반복 보존:** Q00/Q04/Q10-S2025, Q10-S777. Teacher와 같은 seed 조건임을 분리한다.

s3에는 C·TCOPY·CONT·LONG의 원본 전수를 중복 복사하지 않는다. 그 1차 담당은 s2다. 최종 유력한 방법의 확인 반복만 두 서버에 공통 편성한다. 실시간 상태가 새로 바뀌면 완료·실행 중 작업을 먼저 차감한다.

## S3-2. 현재 완료 보고분 — 재학습보다 산출물 검증

| ID | 구성 | HQNR original best | 해당 서버 N0 대비 | 처리 |
|---|---|---|---|---|
| T00 | N0 / OFF | 0.9497 | Teacher | SHEET_REPORTED_50K → local 검증 후 재사용 |
| Q00 | N0 / OFF | 0.9544 | +0.0000 | SHEET_REPORTED_50K → local 검증 후 재사용 |
| Q04 | R3 / OFF | 0.9523 | -0.0021 | SHEET_REPORTED_50K → local 검증 후 재사용 |
| Q06 | R3 / GVH | 0.9566 | +0.0022 | SHEET_REPORTED_50K → local 검증 후 재사용 |
| Q10 | R3 / GVAD | 0.9494 | -0.0050 | SHEET_REPORTED_50K → local 검증 후 재사용 |
| Q01 | R0 / OFF | 0.9543 | -0.0001 | SHEET_REPORTED_50K → local 검증 후 재사용 |
| Q02 | R1 / OFF | 0.9577 | +0.0033 | SHEET_REPORTED_50K → local 검증 후 재사용 |
| Q03 | R2 / OFF | 0.9511 | -0.0033 | SHEET_REPORTED_50K → local 검증 후 재사용 |
| Q35 | N0 / IVH | 0.9527 | -0.0017 | SHEET_REPORTED_50K → local 검증 후 재사용 |
| Q36 | N0 / GCH | 0.9550 | +0.0006 | SHEET_REPORTED_50K → local 검증 후 재사용 |
| Q37 | N0 / SCH | 0.9506 | -0.0038 | SHEET_REPORTED_50K → local 검증 후 재사용 |
| Q07 | R3 / GVWH | 0.9494 | -0.0050 | SHEET_REPORTED_50K → local 검증 후 재사용 |
| Q05 | N0 / GVH | 0.9531 | -0.0013 | SHEET_REPORTED_50K → local 검증 후 재사용 |
| Q11 | N0 / EDGEH | 0.9538 | -0.0006 | SHEET_REPORTED_50K → local 검증 후 재사용 |
...
python tools/na104_hqnr_report.py --grid 10 \
  ...
  --out analysis/final_s3_20260912_core \
...
## S3-4. P1 — s2 후보를 s3에서 완성

| ID | 구성 | 질문/변경 | 직접 대조 | 추가 update | 구분 |
|---|---|---|---|---|---|
| Q09 | R3 / GVFIX | GT 통계 + βV=0.1 고정 Teacher 통계 KD | Q06; Q08; Q10 | 50,000 | 기존 정의 |
| Q12 | R3 / EDGEH | R3 + signed GT edge | Q04; Q06; Q11; Q09 | 50,000 | 기존 정의 |
| Q08 | R3 / GVT | Teacher 통계만. 통계 soft 계수는 원 정의상 1 | Q06; Q09; X08 | 50,000 | 기존 정의 |

Q09는 s2 최고 후보의 직접 반복, Q12는 더 단순한 GT edge 대조의 반복, Q08은 Teacher-only 통계 기준이다. 세 case 모두 s3 T00/best_hqnr와 s3 고정 pilot을 사용한다. s2 checkpoint를 가져와 평가만 한 결과를 s3의 재학습 반복이라고 표시하지 않는다.

공통 평가 격자에서 Q06·Q07·Q08·Q09·Q10을 함께 읽되 원래 best는 보존한다. 특히 Q09와 Q06의 차이가 Teacher 통계의 추가 효과에 가장 직접적이다.

## S3-5. P2 — 구성요소·통계 hard/soft 분리
...
s2와 같은 X 정의를 s3에서 반복한다. R1이 유력했던 s3에서는 X01/X02가 특히 중요하다. R1+구조 감독이 R3+구조 감독과 같다면 reconstruction soft 없이도 충분할 수 있다.

CTLHSCALE은 원래 s2 보류 case지만 s3 R1의 주장에는 s3 안의 대조가 필요하므로 추가 배정했다. X07 R1-shuffle과 함께 Teacher 실패 지도의 위치 정보와 단순 배율을 분리한다.

X05/X06은 새로운 stat 조합 모드이며, 현재 코드에 자동 존재한다고 가정하지 않는다. X08은 Teacher-only 통계의 외부 계수를 0.1로 맞춘다.

**이 단계 다음:** 아래 seed777 core10을 실행한 뒤 다음 P단계로 넘어간다. 준비되지 않은 새 모드 때문에 다른 기존 case를 함께 기다리게 하지 않는다.

## S3-6. P3 — GC·IV·SC 전 모드

| ID | 구성 | 질문/변경 | 직접 대조 | 추가 update | 구분 |
|---|---|---|---|---|---|
| Q25 | R3 / GCH | R3 + GC 통계의 H 감독 | Q04; Q36 | 50,000 | 기존 정의 |
| Q28 | R3 / GCFIX | R3 + GC 통계의 FIX 감독 | Q25 | 50,000 | 기존 정의 |
| Q26 | R3 / GCWH | R3 + GC 통계의 WH 감독 | Q25 | 50,000 | 기존 정의 |
| Q29 | R3 / GCAD | R3 + GC 통계의 AD 감독 | Q26; Q28 | 50,000 | 기존 정의 |
| Q27 | R3 / GCT | R3 + GC 통계의 T 감독 | Q25 | 50,000 | 기존 정의 |
| Q20 | R3 / IVH | R3 + IV 통계의 H 감독 | Q04; Q35 | 50,000 | 기존 정의 |
| Q23 | R3 / IVFIX | R3 + IV 통계의 FIX 감독 | Q20 | 50,000 | 기존 정의 |
| Q21 | R3 / IVWH | R3 + IV 통계의 WH 감독 | Q20 | 50,000 | 기존 정의 |
| Q24 | R3 / IVAD | R3 + IV 통계의 AD 감독 | Q21; Q23 | 50,000 | 기존 정의 |
| Q22 | R3 / IVT | R3 + IV 통계의 T 감독 | Q20 | 50,000 | 기존 정의 |
| Q30 | R3 / SCH | R3 + SC 통계의 H 감독 | Q04; Q37 | 50,000 | 기존 정의 |
| Q33 | R3 / SCFIX | R3 + SC 통계의 FIX 감독 | Q30 | 50,000 | 기존 정의 |
| Q31 | R3 / SCWH | R3 + SC 통계의 WH 감독 | Q30 | 50,000 | 기존 정의 |
| Q34 | R3 / SCAD | R3 + SC 통계의 AD 감독 | Q31; Q33 | 50,000 | 기존 정의 |
| Q32 | R3 / SCT | R3 + SC 통계의 T 감독 | Q30 | 50,000 | 기존 정의 |

GC-H와 GC-FIX를 먼저 수행하되 GC-WH/AD/T, IV와 SC의 모든 모드도 첫 탐색 범위에 포함한다. 표현 단독이 나빴다는 이유로 결합 모드를 영구 생략하지 않는다.

각 표현에서 H↔FIX, H↔WH, WH↔AD를 주 비교로 삼는다. H↔AD만 보고 “Teacher 통계 soft의 효과”로 부르지 않는다. T는 계수가 1이므로 FIX와 순수 target 유무 대조가 아니라는 단서를 유지한다.

서로 다른 표현은 차원·gradient 크기가 다르므로 λ를 표현별 같은 train pilot 방식으로 정한다. 이는 λ를 하나의 숫자로 강제하는 것과 다르며, 표현별 calibration 차이를 보고한다.
...
| Q38 | R3 / GCAD / AOFF_BSIGN_COFF | GC-AD의 방향 선택 | Q29; X09; X10 | 50,000 | 기존 정의 |
| X09 | R3 / GCAD / AOFF_BMASS_COFF | GC B-SIGN의 MASS 대조; 해당 표현 map으로 계산 | Q38 | 50,000 | 신규 정의 |
| X10 | R3 / GCAD / AOFF_BSHUF_COFF | GC B-SIGN의 SHUF 대조; 해당 표현 map으로 계산 | Q38 | 50,000 | 신규 정의 |
| Q44 | R3 / GCAD / AOFF_BCAP_COFF | GC-AD의 B-CAP | Q29; Q38 | 50,000 | 기존 정의 |
| Q45 | R3 / GCAD / AOFF_BCOMPADV_COFF | GC-AD의 성분별 우위 | Q29; Q38 | 50,000 | 기존 정의 |
| Q39 | R3 / SCAD / AOFF_BSIGN_COFF | SC-AD의 방향 선택 | Q34; X11; X12 | 50,000 | 기존 정의 |
| X11 | R3 / SCAD / AOFF_BMASS_COFF | SC B-SIGN의 MASS 대조; 해당 표현 map으로 계산 | Q39 | 50,000 | 신규 정의 |
| X12 | R3 / SCAD / AOFF_BSHUF_COFF | SC B-SIGN의 SHUF 대조; 해당 표현 map으로 계산 | Q39 | 50,000 | 신규 정의 |
| Q46 | R3 / SCAD / AOFF_BCAP_COFF | SC-AD의 B-CAP | Q34; Q39 | 50,000 | 기존 정의 |
| Q47 | R3 / SCAD / AOFF_BCOMPADV_COFF | SC-AD의 성분별 우위 | Q34; Q39 | 50,000 | 기존 정의 |
| Q40 | R3 / GVAD / ASIGN_BOFF_COFF | GV-AD 위의 A만 변경. AB 2×2의 A cell | Q10 | 50,000 | 기존 정의 |
| Q41 | R3 / GVAD / ASIGN_BSIGN_COFF | A+B 결합 | Q10; Q17; Q40 | 50,000 | 기존 정의 |
| Q42 | R3 / GVH_SCH | GV-H + SC-H, 표현별 λ 각각 합산 | Q06; Q30 | 50,000 | 기존 정의 |
| Q43 | R3 / GVAD_SCAD | GV-AD + SC-AD, 표현별 λ 각각 합산 | Q10; Q34; CTLGVSCHALF | 50,000 | 기존 정의 |
| CTLGVSCHALF | R3 / GVAD_SCAD | Q43의 GV·SC λ를 각각 0.5배 | Q43 | 50,000 | 기존 정의 |

Q13/Q17과 GV MASS/SHUF를 s3에서 같이 수행해 A/B 기본 효과를 locally 해석한다. GC/SC 방향 선택에는 X09–X12를 붙인다.

AB 2×2는 **Q10(base), Q40(A only), Q17(B only), Q41(AB)**다. 통계가 없는 Q13을 Q40 대신 넣지 않는다. GV+SC 결합은 Q06/Q30/Q42와 Q10/Q34/Q43을 비교하며, CTLGVSCHALF는 Q43의 두 λ를 모두 절반으로 줄이는 대조다.

COMPADV에 같은 성분 SIGN을 다시 곱한 것을 추가 신규성으로 세지 않는다. 공분산 원소 gate는 좌표축 의존적인 loss이며, 임의의 새 GT/Teacher 혼합 covariance 행렬을 만들지 않는다.
...
| ID | 구성 | 질문/변경 | 직접 대조 | 추가 update | 구분 |
|---|---|---|---|---|---|
| VXW3H | R3 / GVHW3 | GV-H window 3 | Q06 | 50,000 | 기존 정의 |
| VXW3AD | R3 / GVADW3 | GV-AD window 3 | Q10; VXW3H | 50,000 | 기존 정의 |
| VXW7H | R3 / GVHW7 | GV-H window 7 | Q06 | 50,000 | 기존 정의 |
| VXW7AD | R3 / GVADW7 | GV-AD window 7 | Q10; VXW7H | 50,000 | 기존 정의 |
| VXM357H | R3 / GVHW357 | GV-H window 3/5/7 별도 loss 평균 | Q06 | 50,000 | 기존 정의 |
| VXM357AD | R3 / GVADW357 | GV-AD window 3/5/7 별도 loss 평균 | Q10; VXM357H | 50,000 | 기존 정의 |
| VXM2H | R3 / M2H | 비중심 gradient 2차 모멘트 H | Q25 | 50,000 | 기존 정의 |
| VXM2AD | R3 / M2AD | 비중심 gradient 2차 모멘트 AD | Q29 | 50,000 | 기존 정의 |
| VXSTD | R3 / GVADSTD | GV-AD: sqrt(v+1e-12) | Q10 | 50,000 | 기존 정의 |
| VXLOG | R3 / GVADLOG | GV-AD: log(v+1e-9) | Q10 | 50,000 | 기존 정의 |
| VXRES | R3 / GVADRES | GV-AD: 최종 HRMS 대신 residual에서 통계 | Q10 | 50,000 | 기존 정의 |

W3/W7의 H와 AD를 모두 수행하고 기본 W5의 Q06/Q10과 비교한다. MULTI357은 각 창 loss의 평균이며, 창들을 concat한 뒤 하나의 τ로 정규화하는 방식으로 바꾸지 않는다.

STD·LOG는 비음수 GV에만 적용한다. 현재 생성기 epsilon은 STD 1e-12, LOG 1e-9다. GC/SC 원소의 음수에 sqrt/log를 적용하지 않는다. VXRES는 Y−M, T−M, S−M에서 통계를 계산하며 최종 영상 통계와 동일하지 않다.

M2는 비중심 E[ggᵀ], GC는 중심화 covariance다. 같은 full-entry mean L1 reduction으로 Q25↔VXM2H, Q29↔VXM2AD를 비교한다. 각 표현의 실제 통계 오차와 공통 GV-w5 bin 둘 다 남긴다.

이 단계의 원 정의는 대부분 AD 기반이다. 개선된 표현을 다른 reconstruction/FIX에 옮기려면 새 ID와 직접 대조를 만들고, 기존 VX 이름의 의미를 바꾸지 않는다.
...
| 새로 실행 또는 local 상태 확인할 고정 편성 | 83 |
| 합계 | 97 |

이 숫자는 **서로 다른 방법 수가 아니라 server×case×seed×version의 편성 수**다. 새 결과가 이미 완료됐으면 동일 identity를 확인한 뒤 차감한다. 정상 완료분을 큐 목록에 있다는 이유만으로 다시 실행하지 않는다. 추가 confirmation·기술적 재실행은 별도 집계한다.
...
## S3-A. 원래 보류 목록 전수 대응표

| 원 보류 ID | 현재 배치 | 처리 |
|---|---|---|
| Q20 | P3 | 원 정의 보존·1회 full-budget 탐색 |
| Q21 | P3 | 원 정의 보존·1회 full-budget 탐색 |
| Q22 | P3 | 원 정의 보존·1회 full-budget 탐색 |
| Q23 | P3 | 원 정의 보존·1회 full-budget 탐색 |
| Q24 | P3 | 원 정의 보존·1회 full-budget 탐색 |
| Q25 | P3 | 원 정의 보존·1회 full-budget 탐색 |
| Q26 | P3 | 원 정의 보존·1회 full-budget 탐색 |
| Q27 | P3 | 원 정의 보존·1회 full-budget 탐색 |
| Q28 | P3 | 원 정의 보존·1회 full-budget 탐색 |
| Q29 | P3 | 원 정의 보존·1회 full-budget 탐색 |
| Q30 | P3 | 원 정의 보존·1회 full-budget 탐색 |
| Q31 | P3 | 원 정의 보존·1회 full-budget 탐색 |
| Q32 | P3 | 원 정의 보존·1회 full-budget 탐색 |
| Q33 | P3 | 원 정의 보존·1회 full-budget 탐색 |
| Q34 | P3 | 원 정의 보존·1회 full-budget 탐색 |
| Q13 | P4 | 원 정의 보존·1회 full-budget 탐색 |
| Q17 | P4 | 원 정의 보존·1회 full-budget 탐색 |
| Q38 | P4 | 원 정의 보존·1회 full-budget 탐색 |
| Q39 | P4 | 원 정의 보존·1회 full-budget 탐색 |
| Q40 | P4 | 원 정의 보존·1회 full-budget 탐색 |
| Q41 | P4 | 원 정의 보존·1회 full-budget 탐색 |
| Q42 | P4 | 원 정의 보존·1회 full-budget 탐색 |
| Q43 | P4 | 원 정의 보존·1회 full-budget 탐색 |
| Q44 | P4 | 원 정의 보존·1회 full-budget 탐색 |
| Q45 | P4 | 원 정의 보존·1회 full-budget 탐색 |
| Q46 | P4 | 원 정의 보존·1회 full-budget 탐색 |
| Q47 | P4 | 원 정의 보존·1회 full-budget 탐색 |
| CTLGVSCHALF | P4 | 원 정의 보존·1회 full-budget 탐색 |
| VXW3AD | P5 | 원 정의 보존·1회 full-budget 탐색 |
| VXW7AD | P5 | 원 정의 보존·1회 full-budget 탐색 |
| VXW3H | P5 | 원 정의 보존·1회 full-budget 탐색 |
| VXW7H | P5 | 원 정의 보존·1회 full-budget 탐색 |
| VXM357AD | P5 | 원 정의 보존·1회 full-budget 탐색 |
| VXM357H | P5 | 원 정의 보존·1회 full-budget 탐색 |
| VXSTD | P5 | 원 정의 보존·1회 full-budget 탐색 |
| VXLOG | P5 | 원 정의 보존·1회 full-budget 탐색 |
| VXRES | P5 | 원 정의 보존·1회 full-budget 탐색 |
| VXM2H | P5 | 원 정의 보존·1회 full-budget 탐색 |
| VXM2AD | P5 | 원 정의 보존·1회 full-budget 탐색 |

**원 보류 39개 중 누락 0개.** full run name은 동봉 `NA104_S3_FINAL_STAGE_PLAN.json`에 수록했다. 기존 run은 v1 이름을 유지하고, 신규 정의/추가 seed는 v2로 구분한다.
