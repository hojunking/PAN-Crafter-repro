...
**① 단순한 측정 프레임 착시가 아니다 — 그러나 기전은 '프레임' 이 맞다.**
"D_s 가 원 PAN 기준이니 PAN 을 옮기면 당연히 벌점" 이라는 해석은 GT 기준 지표로 한 번 걸러진다 —
`r(|Δ|, ERGAS) = +0.81`(W112 구간), 2.027 → 2.110 으로 GT 대비 복원도 나빠진다.

**후속 실측(2026-09-12 추가)**: 그렇다면 출력을 −Δ 로 되돌리면 복구되는가? P1 의
`full_best_hqnr_mat20.mat` 을 −Δ warp 해 재평가했다(재평가가 기록값 0.93298 을 5자리까지 재현).

| P1 (\|Δ\|=1.511) | HQNR | D_λ | D_s |
|---|---:|---:|---:|
| 원본 출력 | 0.93298 | 0.02157 | 0.04659 |
| 출력을 −Δ 로 되돌림 | **0.89152** | **0.07046** | 0.04097 |

**더 나빠진다.** D_s 는 조금 회복되지만 D_λ 가 3.3배 폭발한다. 즉 출력은 "옳은 영상이 틀린
프레임에 있는" 상태가 **아니다** — 저주파(색)는 MS 에, 고주파(구조)는 warp 된 PAN 에 각각
묶여 있어 전역 shift 하나로 둘을 동시에 고칠 수 없다. 09-09 가 잰 "출력 안에서 구조와 색의
기하가 ~1 px 어긋나 있다" 를 aligner 가 **더 벌린** 것이다.

근본 원인은 `HQNR = (1−D_λ)(1−D_s)` 의 **두 성분이 서로 1.79 px 어긋난 프레임을 기준으로
삼는다**는 데 있다 — D_λ 는 LRMS(MS 프레임), D_s 는 원 PAN(PAN 프레임). 어느 한쪽으로
출력을 보내면 반드시 다른 쪽을 잃는다. 무정합 기준선이 최선인 이유는 학습이 그 사이의
절충점을 이미 찾아놓기 때문이다(09-09: 출력–PAN 거리 1.79 → 0.55 px).
...
**⑤ 다음 — PAN 을 입력단에서 옮기는 방식을 닫는다.**
> **2026-09-12 추기**: 사용자 결정으로 **aligner 컨셉 자체는 유지**한다. 이 판정은
> "입력 PAN 을 warp 하는 구현" 에 대한 것이고, 살아남은 신호(Δ̂ 정확도 84~107%,
> D_λ −35%)를 어디에 쓸지는 [PAN_Aligner_Directions_2026-09-12](../research_log/PAN_Aligner_Directions_2026-09-12.md) 로 정리했다.
1. **입력 PAN 을 warp 하는 구현은 종료한다.** PA·PO10·NF16 계열 후속(반복 pair 포함)을 열지 않는다.
