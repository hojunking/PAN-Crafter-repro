...
**두 모델 모두 α = 1 에서 정확히 최소다.** 그리고 α = −1 은 α = +1 과 **크기·재샘플링 횟수가 완전히
같은데** ERGAS 가 T0 +18 % · P1 +128 % 로 나빠진다 — 따라서 이 변화는 **보간 손실이 아니라
방향이 실어 나르는 정보**다.

**그래서 §13.3 의 "정합 자체가 복원을 개선한다는 증거는 없다" 는 과했다. 정정한다.**
