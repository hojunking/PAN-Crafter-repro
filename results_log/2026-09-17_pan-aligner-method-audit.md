...
## 15. 예시 — aligner 가 PAN 을 실제로 어떻게 바꾸는가 (2026-09-17 추가)

![shift](assets/0917_aligner_shift_example.png)

FR 논문 세트에서 **|c| 가 최대인 scene** 을 골랐다(가장 잘 보이는 경우). 72×72 확대는 PAN 의
경사 에너지가 가장 큰 창. 생성기 `tools/aligner_shift_example.py`.

| 모델 | scene | \|c\| | (dy, dx) | PAN 차분 RMS | 출력 차분 RMS |
|---|---:|---:|---|---:|---:|
| **T0** (λ\* 1e-4, 배포 Teacher) | #19 | **0.564 px** | (+0.508, −0.244) | 0.0545 | 0.0367 |
| **donor N2** (가장 강한 aligner) | #18 | **1.839 px** | (+1.661, −0.789) | 0.2819 | 0.1407 |

**읽는 법**

1. **1·2열(전/후)은 육안으로 구분되지 않는다.** T0 의 0.56 px 는 물론이고 donor 의 1.84 px 도
   전체 그림에서는 티가 안 난다. 이것이 이 method 를 눈으로 검증할 수 없는 이유다.
2. **3열 차분이 실체다.** 변화가 **경계 위에만** 있고 평탄부는 0 이다 — 전형적인 sub-pixel 평행이동
   서명(경계를 사이에 둔 +/− 쌍극자). 균일한 밝기 변화나 흐림이 아니다(§13.3b 의 보간 손실 기각과 일관).
3. **4열 — 출력도 같은 자리에서만 바뀐다.** 출력 차분 RMS 가 PAN 차분의 **약 2/3**(T0 0.0367/0.0545,
   donor 0.1407/0.2819)다. PAN 이동이 출력으로 감쇠돼 전달된다 — 저주파 앵커 `ms_base` 는 움직이지
   않고 U-Net 잔차만 따라가기 때문이다.
4. **5열 단면 — 순수 평행이동임을 확인한다.**

   > **2026-09-17 정정.** 처음에는 "donor 는 값 1.00 이던 밝은 점이 정합 후 0.13 으로 **주저앉는다**"
   > 고 썼다. **진폭이 줄어든 것처럼 읽히는 잘못된 서술이다.** 실제로는 줄지 않았고 **다른 행으로
   > 옮겨갔다.** donor 의 이동은 (dy +1.661, dx −0.789) 로 **세로 성분이 더 크고**, warp 규약은
   > `aligned[y,x] = original[y+dy, x+dx]` 다. 즉 aligned 의 59 행은 원본의 **60.66 행**을 보여준다 —
   > 가로 절단면으로는 세로 이동을 볼 수 없고, 같은 행끼리 비교하면 *다른 내용*을 비교하게 된다.
   > (aligned 59 행의 최댓값은 0.966 으로 원본 1.000 과 사실상 같다. 0.13 은 원본이 봉우리였던
   > **그 열 위치**의 값이지 행의 최댓값이 아니었다.)

   그림에는 검은 점선 — **원본을 `(y+dy, x+dx)` 에서 직접 재샘플한 곡선** — 을 추가했다.
   aligned 곡선과 겹친다:

   | | 재샘플 vs aligned | 같은 행끼리 |
   |---|---:|---:|
   | T0 (dy +0.508) | **r = 0.9998** | r = 0.97 |
   | donor N2 (dy +1.661) | **r = 0.9996** | r = 0.50 |

   재샘플-정합 상관이 0.9996 이상(최대 절대차 0.033)이므로 **진폭 손실 없는 순수 평행이동**이
   맞다. 같은 행 상관이 donor 에서 0.50 으로 떨어지는 것은 세로 이동이 커서 비교 대상이
   달라졌기 때문이며, 이동이 크다는 증거이지 왜곡의 증거가 아니다.

**한 줄**: aligner 는 **평탄부를 건드리지 않고 경계만 sub-pixel 로 미는** 연산이며, 그 효과가
출력에 2/3 강도로 전달된다. 배포 Teacher 의 이동량(0.56 px)은 이 그림에서 보듯 **육안 검증이
불가능한 크기**이고, 그래서 §4·§13 의 정량 검증이 필요했다.
