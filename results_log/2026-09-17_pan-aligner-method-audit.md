...
FR 논문 세트에서 **|c| 가 최대인 scene**. 왼쪽은 맥락(128×128, 노란 상자가 확대 위치),
가운데·오른쪽은 **22×22 px 확대**를 픽셀 블록으로 그린 것. **청록 선은 양쪽 패널에서 완전히
같은 픽셀 행**이라, 구조가 그 선에 대해 이동한 것으로 읽는다. 생성기 `tools/aligner_shift_example.py`.
...
**부호는 합성 임펄스로 실측 확인했다.** 64×64 영상 (32,32) 에 점 하나를 놓고 warp 하면:

| 준 delta (dy, dx) | 점이 간 위치 | 변위 (drow, dcol) |
|---|---|---|
| (+2, 0) | (30.00, 32.00) | (−2.000, +0.000) |
| (0, +2) | (32.00, 30.00) | (+0.000, −2.000) |
| (+1.661, −0.789) = donor | (30.32, 32.82) | (−1.681, +0.819) |

즉 **delta (dy,dx) 를 주면 내용은 (−dy, −dx) 만큼** 움직인다(규약 `aligned[y,x]=original[y+dy,x+dx]`
와 일치). 화면에서 row 감소 = 위, col 증가 = 오른쪽 → 두 모델 다 **위·오른쪽**. 그림의 화살표가 그것이고,
보이도록 3 배로 그렸다.

**읽는 법**

1. **donor(아래 행)는 육안으로 보인다.** BEFORE 에서 청록선에 거의 닿아 있던 밝은 덩어리가
   AFTER 에서 선 위로 떨어지고 오른쪽으로 밀린다 — 화살표 방향과 같다.
2. **T0(위 행)는 경계다.** 0.56 px 는 청록선 대비 자세히 봐야 보이는 정도다.
   **배포 Teacher 의 이동량이 육안 검증의 한계선에 있다는 것이 이 그림의 요지다** —
   그래서 §4·§13 의 정량 검증이 필요했다.
3. **두 모델의 방향이 같고 크기만 3.3 배 다르다.** λ_off 가 바꾸는 것은 *어디로* 가 아니라
   *얼마나* 다 — §3 의 응답 사다리(B −0.20 ~ −0.86)와 일관된다.

> **2026-09-17 정정 이력.** 초판에는 차분 지도와 **가로** 경계 단면을 함께 실었고, donor 단면에
> "값 1.00 이던 밝은 점이 0.13 으로 **주저앉는다**" 고 썼다. **틀린 서술이다** — 진폭은 줄지 않았고
> 내용이 다른 행으로 옮겨갔을 뿐이다. donor 는 세로 이동이 가로의 2.1 배라 aligned 의 59 행은 원본
> **60.66 행**을 보여준다: 가로 단면으로는 세로 이동을 볼 수 없다. 실측 확인 —
> 원본을 `(y+dy, x+dx)` 에서 재샘플하면 aligned 와 **r = 0.9996**(T0 0.9998, 최대차 0.033) 으로
> **진폭 손실 없는 순수 평행이동**이고, 가로 성분만 적용 후 `−dx` 로 되밀면 원본과 r = 0.99898
> (모양 보존), 세로 성분만 적용한 행은 원본 60.66 행과 r = 0.99978(같은 내용) 이다.
> 오해 소지가 큰 차분·단면 패널은 제거하고, 같은 좌표 기준선 방식으로 바꿨다.

**한 줄**: aligner 는 PAN 전체를 **한 방향으로 sub-pixel ~2 px 평행이동**시킨다(왜곡 없음).
배포 Teacher 의 0.56 px 는 22 px 로 확대해야 겨우 보이는 크기다.
