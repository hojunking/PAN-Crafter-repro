...
- 2026-08-28 21:40: **`R3_w112_d124_noattn` 완료 (3/9, 2.24h).** DLPan: ERGAS
  **2.1166** · SCC 0.9903 · Q2n 0.9196, HQNR(공식) **0.9540** @epoch 140.
  c6 대비 HQNR 동급(0.9540 vs 0.9536 — R1 과 동일 값, 이 계열에서 HQNR 는
  전혀 판별하지 못한다)·**ERGAS +1.63%**. **d124 위에서도 폭 축소(128→112,
  params −23%)는 유상이다** — c8 의 depth 교란을 걷어낸 첫 확인점. ERGAS 주력
  게이트(≤2.12)는 통과하나 균형 기준(≤2.10)은 미달. R4(w96) 로 곡선의 다음 점
  확인 중(21:40 개시) — R4 가 c6+1%(2.1034) 를 넘으면 R5(w80) 게이트는 닫힌다.
  best epoch 140 — 후반 패턴은 일관되지 않음(230/205/140).
- 2026-08-28 23:41: **`R4_w96_d124_noattn` 완료 (4/9, 2.01h).** DLPan: ERGAS
  **2.1418** · SCC 0.9900 · Q2n 0.9184, HQNR(공식) **0.9561** @epoch 150 —
  **HQNR 명목 캠페인 최고**인데 ERGAS 는 c6 대비 **+2.84%**. "축소하면 HQNR 이
  자동으로 좋아진다"는 알려진 기전의 전형(판정선 1.18% 미만이라 동급일 뿐).
  **d124 폭 곡선 확정: w128 2.0826 → w112 +1.63% → w96 +2.84% (단조 증가).**
  **R5(w80) 게이트 닫힘 확정**(2.1418 > 2.1034). 한편 c8(w96·d224) 2.2004 대비
  −2.66% — c8 의 붕괴 절반은 depth 몫이었고, d124 가 w96 을 상당히 회복시키나
  주력 게이트(≤2.12)는 여전히 탈락. A1(비대칭) 23:41 개시.
- 2026-08-29 01:15: **`A1_asym_114_10` 완료 (5/9, 1.57h).** DLPan: ERGAS **2.1593** ·
  SCC 0.9899 · Q2n 0.9170, HQNR(공식) 0.9508 @epoch 245. c6 대비 **ERGAS +3.68% —
  캠페인 현재 최악.** enc H/2·dec H/2·dec full 동시 축소의 합산이라 단독 귀속은
  불가하나(§해석 주의), R1(+2.28%)이 full-res 몫을 이미 보여줬으므로 **H/2 블록
  축소분도 ERGAS 를 낸다**고 볼 수 있다. params 이득(2.70M)에 비해 R1(3.00M,
  2.1301)·R3(2.89M, 2.1166)에 ERGAS 로 밀려 **Pareto 열세 가능성** — 추론시간이
  크게 낫지 않으면 탈락권. A2 는 A1 에서 enc full 마저 뺀 것이라 A3 게이트(≤2.12)는
  닫힐 공산이 크다. best epoch 245(사실상 끝). L1_11(LR-Fuse 첫 벌) 01:17 개시.
- 2026-08-29 02:03: **`L1_11_lr_fuse_w64` 완료 (6/9, 0.76h) — LR-Fuse 가설 기각.**
  DLPan: ERGAS **4.0221**(c6 대비 +93%) · SAM 4.60 · SCC 0.9479 · Q2n 0.8543,
  HQNR 0.9293. 초경량 게이트(≤2.25)에 크게 미달 — PixelUnshuffle 이 정보를
  보존해도 **1/16 면적의 w64·6블록 backbone 으로는 full-res 디테일 재구성이
  안 된다**(SAM·Q2n 까지 전면 붕괴 = 스펙트럼·구조 모두 실패). best epoch 240 —
  끝까지 오르는 underfitting 곡선으로 용량 부족과 일관. **L2(w96) 게이트는 사실상
  닫힘 확정**(L1_9 가 2.25 를 넘을 가능성은 희박). 고해상도 연산의 완전 배제는
  이 과제에서 성립하지 않는다 — R1(+2.28%)의 "full-res ResBlock 만 제거" 가
  실질적 하한선이다. L1_9 는 02:03 개시(0.8h 예상, 입력 변인 대조용).
- 2026-08-29 02:49: **`L1_9_lr_fuse_w64` 완료 (7/9, 0.76h).** ERGAS **4.1485** ·
  SAM 4.69 · Q2n 0.8485, HQNR 0.8986. L1_11(4.0221)보다 소폭 나쁨 —
  **LR-Fuse 실패는 입력 변인이 아니라 용량·구조 문제로 확정**(명시적 고주파
  채널은 LR grid 에서도 미미하게만 돕는다). **L2 게이트 닫힘 확정**(4.02 > 2.25).
  A2 는 02:50 개시 — 남은 것은 A2·R6 두 벌, 이후 게이트 판정(전부 닫힐 전망)
  과 DONE. 예상 종료 ~06:00.
