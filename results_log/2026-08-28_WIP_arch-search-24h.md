...
- 2026-08-29 02:49: **`L1_9_lr_fuse_w64` 완료 (7/9, 0.76h).** ERGAS **4.1485** ·
  ...
  **LR-Fuse 실패는 입력 변인이 아니라 용량·구조 문제로 확정**(명시적 고주파
  ...
  과 DONE. 예상 종료 ~06:00.
- 2026-08-29 04:01: **`A2_asym_014_10` 완료 (8/9, 1.18h).** DLPan: ERGAS **2.2657**
  (c6 대비 +8.79%) · SAM 3.04 · SCC **0.9884**(캠페인 SCC 최저, c8 의 0.9893 아래) ·
  Q2n 0.9114, HQNR 0.9492 @epoch 95. **깨끗한 짝 비교 둘이 완성됐다:**
  ① A1→A2(enc full-res 블록 1→0): **+4.93%** — 단일 블록 기준 전 네트워크에서
  가장 비싼 블록. ② R1→A2(H/2 2→1 양측): **+6.37%** — full-res 가 없을 때
  H/2 두 번째 블록이 방어선. **고해상도 인접 용량(full·H/2)이 이 과제의 핵심
  자원이라는 서사가 A·L 계열 전체에서 일관**된다. **A3 게이트 닫힘 확정**
  (2.2657 > 2.12) — 게이트 3개(R5·A3·L2) 전부 닫혔다. R6(마지막) 04:02 개시.
