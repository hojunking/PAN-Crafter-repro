...
| 날짜 | 문서 | 요지 |
|---|---|---|
| 2026-08-25 | [architecture-tuning-review](2026-08-25_architecture-tuning-review.md) | **아키텍처 변경점과 내부 튜닝 전면 검토.** 논문 대비 11/13 반영, params 9.969M→7.1707M(논문 +0.01%). **LayerNorm 이 단일 최대 이득**(−1.34%, 12σ) — 구조 재구성 전체(−0.69%)보다 크다. **시드 2σ 가 재구성본 0.11% vs 배포 코드 1.32%** → **A-1/A-2 의 RR 효과 판정을 철회**(0.74% < 1.32%). 다만 **WV2 zero-shot HQNR 0.9155→0.9345(+2.08%)** 로 fixed 가 명확히 낫다. **ERGAS 최고(paper_ln)와 HQNR 최고(wv3_fixed)가 갈린다** |
