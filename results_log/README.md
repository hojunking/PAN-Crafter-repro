...
| 날짜 | 서버 | 문서 | 요지 |
|---|---|---|---|
| 2026-09-21 | — | [GF2 는 왜 다른가 — aligner 관점](2026-09-21_gf2-aligner-constraint-analysis.md) | 09-20 감사(격차는 recipe 효과)의 후속 — **aligner 가 GF2 에서만 실제로 정합한다.** 같은 추정기 RR 실측: cos(보정, 실제 PAN↔GT) WV3 **−0.398** vs GF2 **+0.812**, 보정 후 PAN↔GT 는 WV3 0.628→0.633(무변) vs GF2 0.515→**0.440**(−15 %). 기전은 aligner 의 MS 기준(bicubic↑MS)이 업샘플 위상을 포함하는데 그 방향이 **GF2 에서만 실제 오정합과 일치**하는 것. 또 Wald 학습쌍 잔여 오프셋이 WV3 0.06 / QB 0.10 / **GF2 0.43 px** 라 GF2 에서는 수축이 사라진다(적용 0.510 / 필요 0.515 = 99 %, WV3 는 22 %). 결과로 D_s 0.0433→0.0225, HQNR +0.021(WV3 는 +0.006). **B02 의 0.967 은 test-set 선택 산물이라 인용 금지**(RR ERGAS 는 오히려 악화). G20 5 run 은 eval 0 회라 미반영. 그림 `assets/0921_gf2_aligner.png`. |
| 2026-09-20 | — | [GF2 데이터셋 실험 분석 — HQNR 격차의 원인 감사](2026-09-20_gf2-hqnr-gap-audit.md) | **지표·데이터는 GF2 에 맞게 이식돼 있다(anchor 5 종 일치: CANConv FR 0.9189/0.0194/0.0629 vs 논문 0.919/0.019/0.063; F-1·F-3 없음; 상수 sweep 최대 +0.0019, D_s 무반응). 격차 0.026–0.033 은 recipe 효과** — 단일-HRMS 4-band W112 P0 가 GF2 에서 PAN 과 과잉 상관(4 밴드×20 장면 전부) 해 D_s 0.0433(dual 0.018–0.023, 논문 0.017); 격차의 74% 가 D_s. 옛 dual W168 3 seed 는 0.958–0.963 으로 논문 수준. 시트 CANConv 행 FR 공란은 eval_version 게이트 표시 문제. B02 0.967 은 aligner 의 PAN 이동 + 선택 폭 0.019 라 이득 아님. 다음: mars×in_mode 2×2 ablation(5.5 h) |
| 2026-09-18 | s1 | [Aligner 크기·정규화·GAP 독립 검증](2026-09-18_s1_aligner-size-norm-gap.md) | **C00–C05 완료: Student3+T0, RR20/FR20, 9모드, 12분16초. 공동목표 통과0/36.** 큰 입력의 q 붕괴 없음; head-order 차이0.00018–0.00032px. G23 seed1234 crop feature-mean의 ERGAS 개선은0.000699(2.070133/HQNR0.959075). GN만 full 통계로 바꾸면 RR 크게 악화, 양쪽 full도 ON을 넘지 못함. 기존 학습 중지 유지·step23230 재개 가능. 그림3개와 원자료 링크 포함. |
| 2026-09-17 | — | [PAN Aligner 검증 — 유의미·정직·제안가능한가](2026-09-17_pan-aligner-method-audit.md) | **현재 형태로는 제안 불가.** aligner 는 진짜 신호를 학습한다(예측 대 proxy 실제 오정합 r=+0.83, 방향 cos 0.992, 20/20 개선) 그러나 (1) **정합을 잘할수록 HQNR 이 나쁘다** — donor 동결 P1(B −0.92, 실제 1.75px 중 1.51 보정)이 0.93298 로 최악, HQNR 최적은 26 % 보정 지점(프레임 비대칭 교환) (2) **scene 적응성이 전역 상수 하나 대비 이득 없음** — learned−const_mean −0.0004~−0.0001, 3 seed 전부 음수(빠져 있던 대조군, 신규 측정) (3) **선택 제거 시 밴드 안** — CTRLP0 대비 best +0.0044 → last +0.0023 (판정선 0.0031) (4) fSCC 일관 악화 −0.017. ε 기전 자체는 작동(B 가 λ 로 −0.20→−0.86 단조)하나 donor(−0.93)를 10K 에 잃고 λ* 는 1/3 만 회복. T0·λ*=1e-4 유지, PAKD50 영향 없음. 그림 A1–A5 `assets/0917_aligner_*`. |
| 2026-09-15 | s1 | [EQREC4 — offset consistency q 의 정체](2026-09-15_s1_eqrec4-results.md) | **q 는 patch 정합 품질의 표지가 아니다.** 같은 checkpoint 안에서 q_A 낮은 patch 는 native 오차가 높고(H1 native 3/3 반대, ρ −0.10…−0.15) 추가 변위에 더 무너진다(H2 3/3 반대); scene 단위(FR20) 에선 q 낮은 scene 이 HQNR 높음(3/3, p<0.05). learned correction 은 zero 보다 +0.0008–0.0012 L1(3/3, 95 % 양) 이고 q 낮을수록 이득 큼. Student cue 로는 판정 불가(H4 diag), 5K pilot 은 **모든 arm 에서 HQNR 0.9555 → 0.951–0.952 하락**(arm 차 ≤0.0009). q_T gate 채택 안 함. 3.7 h.  **[보완 2026-09-15]** 왜 반대인가 — patch 수준에서 **PAN 텍스처가 교란변수**다. 통제하면 ρ(q,e) −0.13 → **+0.28**(checkpoint 7/7 양수, 텍스처 10분위 9/10 양수). EuCd(q↓e↑)=시가지·EdCu(q↑e↓)=식생으로 텍스처 5.7배 차(Cohen's d 2.25). ρ(tex,gain)=+0.678 > |ρ(q,gain)|=0.564 라 H3b 도 텍스처 축. **단 scene 수준은 통제해도 ρ 불변(−0.68→−0.70)** → 원문의 scene 텍스처 귀속 한 문장 정정. 판정·결론 무변경. 그림 S1–S6 `assets/0915_eqrec4_*`. |
| 2026-09-14 | s1 | [PALSV18 결과 + PAKD50 기동](2026-09-14_s1_palsv18-results-and-pakd50-launch.md) | **L1E4(λ_off 1e-4) 양옆 λ 는 3 seed 에서 넘지 못함** — 3e-5 는 3/3 음(−0.0032), 3e-4 는 seed 1234 만 양(+0.0024, 초기 best 16160). working reference L1E4 유지. 정합 반응·보정량은 λ 에 단조(09-13 결론 재현). 이어 L1E4 를 Teacher 로 쓰는 통합 캠페인 PAKD50(J/F × N0/R1/Q12/X02, 3 서버 50h) 기동 (WIP) |
| 2026-09-14 | s1·s2·s3 | [λ* = 1e-4 확정 + NA104 92벌 무소득](2026-09-14_palsv18-lambda-confirmed-and-na104-null.md) | **PALSV18(s1)**: 정점 주변 3e-5·3e-4 를 각 3 seed 로 채운 결과 **λ* = 1e-4 가 양쪽 모두에서 최고** — 평균 0.95597(무정합 대비 **+0.00457**, 부호 3/3 일관, 판정선 초과 2/3)이고 **sd 0.00087 로 최소**. 3e-4 는 평균이 떨어지면서 **sd 4.6배·부호 반전**. **기전 규명**: λ≤1e-4 는 |Δ| 가 학습 내내 제자리(0.48→0.57)인데 **3e-4 부터 50K 에 2배로 드리프트**(0.64→1.10)해 유용창(0.45~0.7)을 벗어난다 — λ* 는 |Δ| 를 창 안에 붙들어 두는 가장 큰 λ. aligner 정확도는 λ 와 함께 계속 좋아지는데(EPE 0.75→0.09) HQNR 은 반대로 간다(재확인). **NA104(s2·s3) 92벌 · 38 case family**: 독립 baseline Q00(0.95355±0.0017) 대비 **판정선 초과 family 0개**, 83% 가 밴드 안 — 정합 없는 KD·출력통계 축은 신호 없음. 미확정: λ*−L000 은 여전히 판정선 초과 1/3, 장면 셔플 무해는 1 case·1 seed |
| 2026-09-13 | s1 | [PALS24 λ_off 스윕 — 정합 축 첫 양성 결과](2026-09-13_s1_pals24-lambda-sweep.md) | **작은 offset loss 가 PAN aligner 를 쓸모 있게 만든다.** λ* = 1e-4 가 무정합 대비 **+0.0044**(3 seed 평균, 판정선 0.0031 의 1.4배), **부호 3/3 일관 · 확인 seed(7777·2025) 둘 다 판정선 초과** → 성립. **HQNR 이 λ 에 비단조** — 1e-4 정점, 1e-3 이상은 −0.005~−0.007 붕괴. λ 는 aligner 를 계속 더 잘하게 만드는데(EPE 0.94→0.23, 입력반응 0.19→0.79) HQNR 은 그 반대로 간다. **09-12 결론 정정**: '옮기면 반드시 잃는다' 는 |Δ|≥1.4 px 구간의 성질이었고, **|Δ| 0.45~0.7 px 에 이득 구간**이 있다(D_s 가 3 seed 전부 개선). 미확정: λ*−L000 은 판정선 초과 1/3 이라 'offset loss 가 핵심' 이라 말할 수 없다. 대가로 fSCC 는 −0.009~−0.024 **+ §7 (같은 날 추가) PALSV18 λ 3e-5/3e-4 × 3 seed 기동 19:28 — WIP, 결과는 09-14 s1 문서** |
| 2026-09-12 | s1 | [정합 축 종합 판정 — PAN 을 옮기는 접근은 성립하지 않는다](2026-09-12_s1_alignment-axis-verdict.md) | **네 캠페인 17벌(BASE96·PA·PO10·NF16) 결론: 정합량 |Δ| 과 HQNR 이 단조 반비례.** W112·D123 안에서 r(|Δ|,HQNR) = **−0.912**, 기울기 −0.0169/px. 두 영역뿐이다 — **|Δ|<0.4 px 는 아무 효과 없고**(PA 3벌·NF16 P2), **|Δ|>1.4 px 는 옮긴 만큼 잃는다**(−0.018~−0.034, 11~22σ). 정합이 이득인 구간은 관측되지 않았다. **결정적: P1(frozen)과 P2(fine-tune)는 같은 donor 에서 출발하는데, 자유 학습한 P2 가 |Δ| 를 1.511→0.344 로 77% 줄이고서야 HQNR 이 무정합 수준으로 회복됐다 — 최적화의 답이 '옮기지 말라'였다.** **출력을 −Δ 로 되돌리면 더 나빠진다**(0.933→0.892, D_λ 3.3배 폭발) — 저주파는 MS 에, 고주파는 warp 된 PAN 에 묶여 전역 shift 로 못 고친다. 근본 원인은 HQNR 의 두 성분이 1.79px 어긋난 프레임(D_λ↔LRMS, D_s↔원 PAN)을 기준삼는 것. **aligner 컨셉은 유지**하고 방향은 [PAN_Aligner_Directions](../research_log/PAN_Aligner_Directions_2026-09-12.md) 로 정리. **부산물: HQNR 시드 판정선을 처음 실측 — 2σ = 0.0031**(3 경로 일치; 종전 0.011 은 3.5배 과대). PAN 을 옮기는 축 종료, MS 조건 입력 쪽만 남긴다 **+ §8 (같은 날 추가) PALS24 λ_off 비교 캠페인 기동 13:00 — WIP, 결과는 다음 s1 문서** |
