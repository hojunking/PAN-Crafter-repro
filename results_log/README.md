...
| 날짜 | 문서 | 요지 |
|---|---|---|
| 2026-09-01 | [ms-only-ablation-s2](2026-09-01_ms-only-ablation-s2.md) | **소형 모델에서 MARs 는 비용이다.** R4(2.13M)에서 단일-task 가 dual 을 **ERGAS −2.68%**(4벌 전부 20/20, dual 대조군 2벌 기준 Δ0.00%)로 이기고 **학습 1.6배 빠르다**. **모드 변조 파라미터는 무가치**(n.s.) → Student 기본형은 MS2(plain). HQNR 무승부. 적대적 감사로 기계적 교란 전부 기각. **단 구조 의존** — 같은 ablation 이 w128 에선 −0.64% 라 "작을수록 보조 task 부담이 크다"로 읽는다. 배포본 MARs 반박 아님 |
