...
check("K26 s5 명시 순서(§5): J0→JQ→QE50 @W104 S2026 → J0→JQ→QE50 @W104 S777(run 이름 항목) · 전부 QEDGE9 branch · 777 은 허용 seed(확인 seed 아님; 9091 은 확인) · 예약(EDGEBAL §7.1 실측 1.38/1.57/1.65 로 교체) 1.6847/1.8937/1.9817 ×2 = 11.1200 h · 9091 은 EDGEBAL 허용 seed(K39) · 옛 순서는 PREVIOUS['s5_20260915'] · cue 없는 QEC 는 cue_ready False",
      ...
      and all(G.branch_for("s5", it) == "QEDGE9" for it in _p5[:6]) and G.allowed_seeds("s5") == {2026, 777, 9091, 1103} | set(G.QRC24_R2_SEEDS["s5"]) and abs(sum(_r("s5", it)["reservation_h"] for it in _p5[:6]) - 11.12) < 1e-6
...
check("K26 s4 앞 5 는 기존 E0 allocation(NA0→J0→JQ→XJ→F0 @W104, 마감 상속; QEDGE9 항목 없음 — QEGX 14 는 K34) · JQ/XJ/F0 1.6847/1.6737/1.4097 = 4.7680 h · 확인 seed 3407 은 이제 QEGX 명시 표(allowed)·옛 --confirm 규칙은 그대로",
      ...
      and abs(sum(_r("s4", it)["reservation_h"] for it in _p4[2:5]) - 4.7680) < 1e-4 and G.allowed_seeds("s4") == {1234, 3407} | set(G.QRC24_R2_SEEDS["s4"]) and G.confirmation_cases("JQ", "s4") == ["J0@W104_D121", "JQ@W104_D121", "J0", "JQ"])
check("K26 s1 명시 순서(17:20 s4→s1): J0→JQ→QE50→QES→QEC @W104 S1234 **v2**(run 이름 항목; E0 v1 config 와 충돌 없음) · 전부 QEDGE9 branch · QEC 마지막(pilot = s1 J0 v2 exact50K) · 예약 1.4647/1.7067/1.8167/1.8167/1.7067 = 8.5113 h · 허용 seed {1234} · v2 config 의 work_dir·baseline 도 v2",
      _p1 == [G.run_name(c, 1234, "v2", arch="W104_D121") for c in ("J0", "JQ", "QE50", "QES", "QEC")] and all(G.branch_for("s1", it) == "QEDGE9" for it in _p1) and G.allowed_seeds("s1") == {1234, 3407} | set(G.QRC24_R2_SEEDS["s1"])
...
check("K34 편성(§5·§6): s3 = 15 run **v2**(2026 A/B 10 → 4321 C 5) · s4 = E0 5 뒤 14 run v1(1234 A 8 → 3407 C 6) · 전부 QEGX branch · allowed seed s3 {2026,4321} / s4 {1234,3407} · 옛 s3 순서는 PREVIOUS · bare 'QE50@W104_D121' 은 QEDGE9 자동 규칙(큐에는 run 이름만) · 큐 파일 == 순서 · refresh v3 는 QEGX",
      _p3 == _want3 and _p4x == _want4 and all(G.branch_for("s3", it) == "QEGX" for it in _p3) and all(G.branch_for("s4", it) == "QEGX" for it in _p4x) and G.allowed_seeds("s3") == {2026, 4321} | set(G.QRC24_R2_SEEDS["s3"]) and G.allowed_seeds("s4") == {1234, 3407} | set(G.QRC24_R2_SEEDS["s4"])
...
check("K39 편성(§5·§6): s2 = 12 run v1(777 A 6 → 2026 B 3 → 9091 C 3) · s5 = QEDGE9 6 그대로 뒤 14 run **v2**(2026 A 8 → 777 B 2 → 9091 C 4) · 전부 EDGEBAL branch(s5 앞 6 은 QEDGE9 유지) · allowed seed s2/s5 {777, 2026, 9091} · 큐 파일 == 순서 · 옛 s2 순서 PREVIOUS · QF* 는 cue case · λE 의존(EB_N0/EB_R3E000 제외)",
      ...
      and G.allowed_seeds("s2") == {777, 2026, 9091} | set(G.QRC24_R2_SEEDS["s2"]) and G.allowed_seeds("s5") == {2026, 777, 9091, 1103} | set(G.QRC24_R2_SEEDS["s5"]) and G.EDGEBAL_ITEMS_BY_SERVER == {"s2": _w2, "s5": _w5} and G.PREVIOUS_PRIORITY_BY_SERVER["s2_20260916_edgebal"] == _p2
...
for _lab, _fn in (("K44 서버 토큰 ≠ 생성 서버 → SystemExit", lambda: G.kdv_block("QRC24_S3_G22", 2026, "s5", cal=calQ, arch="W104_D121")), ("K44 QRECON24 는 W104 전용 (W112 거부)", lambda: G.kdv_block("QRC24_S2_G22", 777, "s2", cal=calQ)),
    ...
    try:
        ...
        check(_lab, True)
_rsum = lambda srv: sum(G.reservation_for(srv, it)["reservation_h"] for it in G.qrc24_items(srv))          # 계획 §8 의 합은 ADJ-R1 편성분 (R2 는 아래에서 같은 식으로 따로)
check("K44 편성·예약(§5·§8; ADJ-R1 활성 편성 = 등록 + 남은 순서, 보류 제외): PRIORITY 다섯 서버 전부 QRC24 run 이름(= qrc24_items) · branch QRECON24 · allowed seed(현재 + superseded branch 의 명시 seed) s1 {1234,3407} s2 {777,2026,9091} s3 {2026,4321} s4 {1234,3407} s5 {2026,777,9091,1103} · 예약 1.20×R_s + 10/60 (R_s 는 ADJ-R1 §10 대용 — v1·v2 공통, s1 A_FREEZE 2.15; measured 우선) → s1 42.34 / s2 38.86 / s3 38.61 / s4 37.59 / s5 52.20 h (kind plan_reference_qrc24_adj_r1, slack 1.2) · 옛 순서는 PREVIOUS · 큐 파일 == 순서 · cue 필요 · §8.3 확장 3 run",
      all(G.priority_for(s_) == G.qrc24_items(s_) + G.qrc24_r2_items(s_) and all(G.branch_for(s_, it) == "QRECON24" for it in G.priority_for(s_)) for s_ in ("s1", "s2", "s3", "s4", "s5")) and [G.allowed_seeds(s_) for s_ in ("s1", "s2", "s3", "s4", "s5")] == [b_ | set(G.QRC24_R2_SEEDS[s_]) for s_, b_ in zip(("s1", "s2", "s3", "s4", "s5"), ({1234, 3407}, {777, 2026, 9091}, {2026, 4321}, {1234, 3407}, {2026, 777, 9091, 1103}))]
      and all(abs(_rsum(s_) - v_) < 1e-3 for s_, v_ in (("s1", 12 * 3.070667 + 2 * 2.746667), ("s2", 12 * 3.238667), ("s3", 20 * 1.930667), ("s4", 19 * 1.978667), ("s5", 16 * 3.262667))) and all(G.reservation_for(s_, r_)["slack"] == 1.2 and G.reservation_for(s_, r_)["reservation_h"] > 0 for s_ in ("s1", "s2", "s3", "s4", "s5") for r_ in G.qrc24_r2_items(s_)) and G.reservation_for("s4", G.priority_for("s4")[0])["reference_kind"] == "plan_reference_qrc24_adj_r1" and G.reservation_for("s4", G.priority_for("s4")[0])["slack"] == 1.2
...
_k48 = dict(n81=(len(_allQ) == 81 + sum(len(G.qrc24_r2_items(s_)) for s_ in ("s1", "s2", "s3", "s4", "s5"))),          # ADJ-R1 81 + Narrow R2 (seed 20 + β-close 4) exist=all(v_ is not None for v_ in _cfgQ.values()), meta=all(v_["train_feeder_args"].get("return_meta") is True and v_["kdv"]["campaign_id"] == G.QRC24_CAMPAIGN_ID and "training_deadline" not in v_["kdv"]["budget"] and v_["kdv"]["qrecon"]["mode"] == "continuous_v1" for v_ in _cfgQ.values()),
...
check("K50 편성(§5·§9·부록 B): 남은 순서 s1 G22@3407→G23@3407→A03_UNIF@1234 v2→A03_SHUF@1234 v2→A_UNIF→A_FREEZE→A_SHUF→G21@3407 · s2 G13→E_UNIF→E_SHUF→ALL_UNIF→G31→G33@777 · s3 G23@4321→H23 v2→B20A03 v2→G13→G12→G21→G11→G32→G31→G33 · s4 H_BETA0@1234→H22@3407→H23 v2→H12→H13 v2→H32 v2→H33 v2→G23@1234 v2→B20A03@1234 v2 · s5 L070→L050@9091→G23/H23/B20A03@9091 v2→L100@1103→G23/H23/B20A03@1103 v2 · v2 16 == 부록 B · 유지 26 v1 ⊂ 원계획 · 보류 s4 H11@3407, s5 L070/L050@1103 (priority·mandatory 밖) · 등록 39 · 활성 14/12/20/19/16 = 81 · 큐 파일 == 활성",
      ...
      and [len(G.qrc24_items(s_)) for s_ in ("s1", "s2", "s3", "s4", "s5")] == [14, 12, 20, 19, 16] and all(G.qrc24_items(s_) == [G.qrc24_run_name(s_, p_, sd_, v_) for p_, sd_, v_ in G.QRC24_REGISTERED[s_]] + [G.qrc24_run_name(s_, p_, sd_, v_) for p_, sd_, v_ in G.QRC24_ADJ_ORDER[s_]] for s_ in ("s1", "s2", "s3", "s4", "s5")) and all(G.priority_for(s_) == G.qrc24_items(s_) + G.qrc24_r2_items(s_) and len(G.qrc24_r2_items(s_)) == 4 + len(G.QRC24_R2_BETA_CLOSE.get(s_, [])) for s_ in ("s1", "s2", "s3", "s4", "s5")) and all(v_ == "v1" for l_ in G.QRC24_REGISTERED.values() for _, _, v_ in l_)
...
try:
    ...
    _mand_all = G.priority_for("s2") + [_keep_run]; _pend5 = [r_ for r_ in _mand_all if not _term5(r_)]     # 2026-09-18: 편성 = ADJ-R1 + Narrow R2
...
check("K52 β-close·큐(§4.1·§4.2·§8.1): 새로 도는 B20A03 은 **기존 G23 의 짝만** 4 개(s1 1234/3407 · s2 777 · s3 2026; s3 4321 은 이미 있고 s4 는 G23 host bridge 로 끝) · 편성 이력 표기는 v3 이고 수식 변경이 아니다 · 활성 실행 순서 꼬리에 R2 가 붙는다 · **R2 run 은 config 만 있는 게 아니라 운영 편성(priority_for)·서버 큐 파일에 실제로 들어 있다**(2026-09-18: **d362e57** 이 R2 24 run(seed 20 + β-close 4) 을 qrc24_effective_queue 에만 붙이고 PRIORITY_BY_SERVER·큐 파일에 넣지 않았고 a565caf 가 그대로 지나쳤다 — s1 이 5 h 43 m 유휴. 그 회귀를 여기서 막는다)",
      ...
      and all(r_ in G.priority_for(s_) and r_ in _qf52[s_] and os.path.exists(os.path.join(ROOT, "config", r_ + ".yaml")) for s_ in ("s1", "s2", "s3", "s4", "s5") for r_ in G.qrc24_r2_items(s_))
      and all(_qf52[s_] == G.priority_for(s_) and len(G.priority_for(s_)) == len(G.qrc24_items(s_)) + len(G.qrc24_r2_items(s_)) for s_ in ("s1", "s2", "s3", "s4", "s5")))
...
_gu52m = _ilu52.module_from_spec(_ilu52.spec_from_file_location("_gu52m", os.path.join(ROOT, "gspread", "gspread_upload.py"))); sys.modules["_gu52m"] = _gu52m; _ilu52.spec_from_file_location("_gu52m", os.path.join(ROOT, "gspread", "gspread_upload.py")).loader.exec_module(_gu52m)
_clean52 = _ilu52.module_from_spec(_ilu52.spec_from_file_location("_clean52", os.path.join(ROOT, "gspread", "sheet_cleanup.py"))); _ilu52.spec_from_file_location("_clean52", os.path.join(ROOT, "gspread", "sheet_cleanup.py")).loader.exec_module(_clean52)
check("K52 시트 표시명 단축(2026-09-18 사용자 결정): 고정 method 는 접두어 'QRC24' 하나로 포괄하고 뒤에 profile·seed(·v2 이상이면 version) 만 — 서버·골격·프로토콜은 탭과 고정 method 가 말한다 · 짧은 표기 ↔ 원래 실행명이 왕복 복원된다 · **긴 표기와 짧은 표기가 같은 canonical key** 라 이미 올라간 행을 중복 생성하지 않는다 · 긴 실행명은 Notes(run=…) 에 남는다 · QRC24 가 아닌 run 은 그대로 · **업로더(compose_cells)와 정리 도구(sheet_cleanup)가 같은 모양을 만든다**(다르면 다음 업로드가 B 를 도로 늘린다) · 구분행은 정리 대상이 아니다",
      ...
      and "canonical_run_key(t, server)" in _gu52
      and all("canonical_run_key(cell, srv)" in open(os.path.join(ROOT, "gspread", f_)).read() for f_ in ("noa_upload.py", "target_upload.py"))
      and _gu52m.compose_cells(_long52, "method=…", "seed=41001", "50K") == ("QRC24 G23 S41001 (50K)", f"run={_long52} · method=… · seed=41001")
