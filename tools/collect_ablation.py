...
def main():
    prof = {r["tag"]: r for r in json.load(open(f"{ASSET}/ablation_profile.json"))}
    ...
    for tag, p in prof.items():
        r = eval_one(f"{ROOT}/work_dir/abl_{tag}", gt, lms_r, lms_f, pan_f, wald)
    ...
    json.dump(rows, open(f"{ASSET}/ablation_results.json","w"), indent=1)
...
def write_doc(rows, base, ndone):
    ...
    if ndone >= 2:
        L += ["![](assets/ablation_plot.png)", "*성능 대비 비용. 좌하단이 유리하다.*", ""]
...
def make_fig(done, base):
    ...
    fig.tight_layout(); fig.savefig(f"{ASSET}/ablation_plot.png", dpi=130); plt.close(fig)
