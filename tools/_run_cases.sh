...
case "$FH20R1_SERVER" in s1|s2|s3|s4|s5)
    if [ -f "$REPO/work_dir/_fh20r1/$FH20R1_SERVER/campaign_budget.json" ]; then
        ...
        [ -x "$PY" ] || PY=python
        exec "$PY" tools/fh20r1_runner.py run --server "$FH20R1_SERVER"
    fi;;
esac
[ ! -e "$REPO/work_dir/_fh20r1/local_server.txt" ] || { echo "FH20R1 registration incomplete; refusing legacy fallback"; exit 0; }
