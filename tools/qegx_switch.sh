...
srv = sys.argv[1]; xp = os.path.join(G.ROOT, G.EXTRA_PRIORITY_FILE); cur = G.extra_priority(); old = [x for x in cur if G.branch_for(srv, x) != "QEGX"]; keep = [x for x in cur if G.branch_for(srv, x) == "QEGX"]   # 감사 F04: 옮기기 전에 메모리에 읽는다
if old:
    bak = xp.replace("extra_priority.txt", f"extra_priority.pre_qegx_{time.strftime('%m%d-%H%M')}.txt"); os.replace(xp, bak)
    open(xp, "w").write(f"# QEGX 전환({time.strftime('%Y-%m-%dT%H:%M')}): 이전 추가 편성 {len(old)} 항목은 {os.path.basename(bak)} 에 보존 — QEGX 의 추가 편성(대조 새로고침 v3 등) 만 여기에\n" + "\n".join(keep) + "\n")
    print(f"   extra_priority {len(old)} 항목(비 QEGX) → {os.path.basename(bak)} (보존; 편성에서 제외)")
