...
def native_proxy(m, ds, pan_raw, sensor, dev):
    ...
    for i in range(len(ds)):
        ...
        rows.append(dict(scene=i, delta_dy=float(d[0]), delta_dx=float(d[1]), delta_norm=float(d.norm()), before_dy=a["dy"], before_dx=a["dx"], before_mag=a["mag"], before_zncc=a["zncc"], before_accepted=a["accepted"], before_boundary=a["boundary"],
                         after_dy=b["dy"], after_dx=b["dx"], after_mag=b["mag"], after_zncc=b["zncc"], after_accepted=b["accepted"], after_boundary=b["boundary"], improved=bool(b["mag"] < a["mag"]), both_accepted=bool(a["accepted"] and b["accepted"])))
    ...
    summ = dict(n=len(rows), n_both_accepted=len(acc), before_mag_median=float(np.median([r["before_mag"] for r in rows])), after_mag_median=float(np.median([r["after_mag"] for r in rows])),
                ...
                identity_check=ident, known_shift_check=dict(applied_dy_dx=[1.0, 0.0], measured=known, note="sign convention of the audit vs W(P,c)[y,x]=P[y+cy,x+cx] recorded here; audit is PAN_b <- up(MS) on the HR grid"),
...
def interventions(m, ds, lms_raw, pan_raw, sensor, wald, mp, dev, const_vec, sigma_star, blur_status):
    ...
    for mode, fn in list(modes.items()) + ([("blur_energy_match", None)] if sigma_star is not None else []):
        ...
        for i in range(n):
            ...
            v, ok, _ = scene_views(sr, lms_raw[i].transpose(1, 2, 0), p, pa_eval, sensor, wald, d, 4, mp); ok_n += int(ok); per.append(v["raw_original"]["hqnr"])
            for k in VIEWS:
                for q in acc[k]:
                    acc[k][q].append(v[k][q])
        out[mode] = dict(views={k: {q: float(np.mean(acc[k][q])) for q in acc[k]} for k in VIEWS}, per_scene_raw_hqnr=per, n_eligible=ok_n, applied=("sigma %.2f, correction 0 (%s)" % (sigma_star, blur_status) if mode == "blur_energy_match" else mode))
...
def main():
    ...
    ap.add_argument("--run", required=True); ap.add_argument("--ckpt", default="best_hqnr"); ap.add_argument("--parts", default="v2,v3,v4"); ap.add_argument("--stress-dirs", type=int, default=4, choices=(4, 8))
    ...
    summ = dict(run=a.run, ckpt=a.ckpt, step=meta.get("step"), ckpt_sha256=sha256_file(os.path.join(wd, a.ckpt, "model.safetensors")), aligner=is_pa, radius_hr=R, view_margin=mg, parts=sorted(parts))
    ...
    if "v3" in parts:
        if is_pa:
            rows, ns = native_proxy(m, dsf, pan_raw, sensor, dev); write_csv(os.path.join(od, f"{a.ckpt}_native_proxy.csv"), rows); summ["v3_native_proxy"] = {k: v for k, v in ns.items() if k not in ("identity_check", "known_shift_check")}
            print(f"  V3.1 native proxy |δ| median before {ns['before_mag_median']:.3f} → after {ns['after_mag_median']:.3f} HR px (accepted {ns['n_both_accepted']}/20, improved {ns['n_improved']}/20) | identity {ns['identity_check']['mag']:.3f} known (1,0) → ({ns['known_shift_check']['measured']['dy']:+.2f},{ns['known_shift_check']['measured']['dx']:+.2f})")
