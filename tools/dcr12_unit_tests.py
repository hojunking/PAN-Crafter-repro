...
need = ("meta/config.yaml", "best_hqnr/model.safetensors", "best_hqnr_meta.json", "last/model.safetensors", "last_meta.json", "results/fr_mat20.json", "checkpoint_metrics.csv") + tuple(f"candidates/step-{st}/model.safetensors" for st in sorted(set(C.D03_STEPS + C.D04_STEPS)))
check("X14 pair bundle 정의: seed pair 의 B0/B1 두 run · 필수 = config/best_hqnr/last(+meta)/fr_mat20/checkpoint_metrics/candidates(D03∪D04 격자) · .mat 은 --with-mats 일 때만(기본 sha/size 증거) · optimizer.bin 제외 · trained_on 은 provenance 없으면 이 서버", {e["run"] for e in ent} == {rB0, rB1} and all((r, f) in req for r in (rB0, rB1) for f in need) and not any(e["file"].endswith(".mat") for e in ent) and all(e["file"].endswith(".mat") for e in BND.entries(1234, True) if e["file"].startswith("results/") and e["file"] != "results/fr_mat20.json") and not any("optimizer" in e["file"] for e in ent) and C.trained_on("B0", 1234) in (C.SERVER, None) and C.host_pairing()["note"].startswith("§8.2"))
