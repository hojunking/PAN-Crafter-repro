...
w_i = sg( 2·qref / (qref + q_T(i)) ),  qref = 0.3276133416220546 (T0/train/AXIS16 calibration 중앙값을 scale 로 재사용; threshold 아님).
q_T 는 cue 자산(assets/qedge9/cue_T0_AXIS16_v1.{json,npz}) 의 **raw q**(sample index × 실제 rot state) — 기존 gate_low_q 0/1 표에 함수를 씌우지 않는다.
q ≥ 0·유한이 아니면 fail-fast, w(qref) = 1, w(0) = 2, 0 < w ≤ 2. batch 정규화·threshold·temperature·clip 없음.
...
def q_weight(q, qref):
    """w = 2·qref/(qref + q) (float64). q 는 유한·비음, qref > 0 — 아니면 ValueError (조용히 clip 하지 않는다)."""
    qref = float(qref)
...
class QWeight:
    ...
    def __init__(self, qref, tables, a_mode, e_mode, manifest=None, asset=None, checks=None, stats=None):
        for m in (a_mode, e_mode):
            if m not in MODES:
                raise ValueError(f"qrecon weight mode {m!r} ∉ {MODES}")
        self.qref = float(qref); self.tables = tables; self.a_mode = a_mode; self.e_mode = e_mode; self.manifest = manifest or {}; self.asset = asset; self.checks = checks or {}; self.stats = stats or {}
        self._dev = {}
    ...
    def load(cls, cfg, teacher=None, train_h5=None, feeder_args=None, sha_fn=None, previous=None):
        ...
        return cls(qref, tables, cfg.get("a_weight", "q"), cfg.get("e_weight", "q"), manifest=man, asset=asset, checks=checks, stats=stats)

    @classmethod
    def synthetic(cls, table_q, table_shuffle=None, qref=QREF_DEFAULT, a_mode="q", e_mode="q"):
        """검사용: (index, rot) 표를 직접 준다 (w 값 자체)."""
        return cls(qref, {"q": table_q, "shuffle": (table_shuffle if table_shuffle is not None else table_q)}, a_mode, e_mode, manifest=dict(synthetic=True))
    ...
    def weight_for(self, meta, device, mode):
        """meta [B,4] int64 = (index, rot, hflip, vflip) → w [B] float32 (sg). mode ∈ MODES; uniform 은 1."""
        if mode == "uniform":
            if meta is None:
                raise ValueError("qrecon: batch meta(index, rot) 가 필요하다 — train_feeder_args.return_meta: true")
            return torch.ones(int(meta.shape[0]), dtype=torch.float32, device=device)
        if meta is None:
            raise ValueError("qrecon: batch meta(index, rot) 가 필요하다 — train_feeder_args.return_meta: true")
    ...
    def summary(self):
        ...
        return dict(mode="continuous_v1", formula="w_i = 2*qref/(qref + q_T(i)); q_T = frozen T0 aligner AXIS16 raw q (cue asset), sg", qref=self.qref, a_weight=self.a_mode, e_weight=self.e_mode, asset=self.asset,
