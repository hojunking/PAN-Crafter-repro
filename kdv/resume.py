...
def begin_epoch(loader, es, global_step, resume_pending):
    ...
    n = len(loader); info = dict(exact=False, skipped=0, epoch_start_step=None)
    if resume_pending and es is not None and es.epoch_rng is not None and es.epoch_start_step is not None:
        ...
        if not (0 <= skip < n) or int(es.n_batches or n) != n:
            raise RuntimeError(f"exact resume: checkpoint 의 epoch 상태(start {es.epoch_start_step}, n_batches {es.n_batches}) 와 global_step {global_step}/loader {n} 가 맞지 않는다 — 처음부터 다시 돌린다")
