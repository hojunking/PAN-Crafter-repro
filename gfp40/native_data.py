...
def bind_lane_data(original, server, root):
    ...
    if server not in ('s3', 's4', 's5'):
        raise ValueError('GFP40 lane data cannot bind protected s1/s2')
    if not isinstance(original, dict) or original.get('schema') != 'G20_DATA_v1':
        raise ValueError('P40 registers only original L100/G20 native100K data; no old50K parent')
    ...
    provenance = dict(schema='GFP40_NATIVE_LANE_WRAPPER_v1', server=server,
