...
REGISTRY_PATH = 'gfp40/parent_registry.json'
SCHEMA = 'GFP40_PARENT_BINDING_v1'
REFERENCE_SCHEMA = 'GFP40_NATIVE_REFERENCE_BINDING_v1'
...
NUMERICAL_PATHS = ('fh12/model.py', 'fh12/data.py', 'fh12/calibration.py',
    ...
    'pa/aligner.py', 'pa/warp.py', 'fh12/losses.py', 'qg40/losses.py',
    'g20/model.py', 'g20/losses.py', 'g20/data.py', 'g20/evaluation.py',
    'tools/eval_dlpan.py', 'tools/metrics/eval_rr.py', 'tools/metrics/eval_fr.py',
    'tools/metrics/q2n.py', 'tools/metrics/jqm.py', 'tools/repair_lpan.py')
RUNTIME_KEYS = ('torch', 'numpy', 'scipy', 'skimage', 'cuda', 'cudnn',
                'tf32_matmul', 'tf32_cudnn')


def registry(root=ROOT):
    return read_json(Path(root) / REGISTRY_PATH)
...
def _field(cfg):
    found = [cfg[k] for k in ('l100', 'qg40', 'g20', 'gfb20', 'gfp40') if k in cfg]
    if len(found) != 1:
...
def source_compatibility(origin, root=ROOT):
    ...
    from gfp40.common import source_identity
    ...
    changed += [name for name, digest in origin['files'].items()
                if name.startswith('external/') and current['files'].get(name) != digest]
...
def discover_bindings(server, root=ROOT):
    ...
    if server not in ('s3', 's4', 's5'):
        raise ValueError('GFP40 is restricted to s3/s4/s5')
    root = Path(root)
    result = {}
...
def _data_check(data, root):
    if (data.get('sensor') != 'GF2' or data.get('num_bands') != 4
            ...
            or {key: value.get('count') for key, value in data['splits'].items()}
               != dict(train=19809, val=2201, rr=20, fr=20)):
...
def _reference_binding(path, reference_key, server, spec, root, origin_root, local_data=None):
    ...
    result = dict(schema=REFERENCE_SCHEMA, native_reference_id=reference_key,
        ...
        teacher_compute={key: full.get(key) for key in
                         ('training_seconds', 'evaluation_seconds', 'io_seconds')})
...
def bind_parent(parent_id, server, binding, root=ROOT, *, local_dataset_manifest=None):
    ...
    if (Path(cfg['work_dir']).name != spec['source_run_id'] or cfg.get('seed') != spec['parent_seed']
            ...
            or args.get('depth') != [1, 2, 2] or field.get('profile') != 'BASE'
            or (field.get('alpha'), field.get('beta'), field.get('lambda_edge')) != (1., .1, .002)
            or cfg.get('num_iter') != 100000):
...
def verify_parent(value, root=ROOT, *, local_dataset_manifest=None):
    ...
    if value.get('parent_kind') == 'GFP40_FRESH_TRUNK':
...
def bind_fresh_parent(parent_id, server, config_path, root=ROOT):
    """Bind a registered local fresh100K trunk, never a best model or FT tail."""
    from gfp40.common import read_config, source_identity
    from gfp40.plan import case_for, validate_config
    from gfp40.plan import CASES
    ids = {c.case_id for c in CASES if not c.is_ft and c.updates == 100000 and c.server == server}
    if parent_id not in ids:
        raise ValueError('Only registered same-server fresh100K trunks can become new parents')
    cfg_path = _path(config_path, root)
    cfg = read_config(cfg_path)
    case = validate_config(cfg, require_bound=True)
    if case != case_for(parent_id) or case.updates != 100000 or case.is_ft:
        raise ValueError('Fresh parent config is not the declared fresh100K trunk')
    ...
    full = _fullstate(full_path, identity, tensors)
    data_path = _path(cfg['gfp40']['dataset_manifest'], root)
    data = read_json(data_path)
    if identity.get('data_sha256') != object_sha(data):
    ...
    native = verify_reference(_path(cfg['gfp40']['reference_manifest'], root), root,
                              local_dataset_manifest=data)
    if native['native_reference_id'] != case.reference_key or native['server'] != server:
        raise ValueError('Fresh trunk reference is not its registered local reference')
    ...
    summary = read_json(run / 'official/summary.json')
    row = summary.get('selections', {}).get('EXACT_FINAL', {})
    if (summary.get('official_complete') is not True
            or summary.get('run_id') != case.run_id or summary.get('case_id') != case.case_id
            or summary.get('config_sha256') != object_sha(cfg)
            or row.get('update') != 100000 or row.get('checkpoint_identity') != identity):
        raise ValueError('Fresh trunk must be officially complete and preserved before a fork')
    validate_record(row, 'GF2')
    model, _ = build_model('PLH', 104, [1, 2, 2], case.seed, role='S', num_bands=4,
        teacher_aligner_state={k[len('aligner.'):]: v for k, v in tensors.items() if k.startswith('aligner.')})
    model.load_state_dict(tensors, strict=True)
    return _signed(dict(schema=SCHEMA, parent_kind='GFP40_FRESH_TRUNK', parent_id=parent_id,
...
def load_reference(value, device='cpu', root=ROOT, *, local_dataset_manifest=None, deadline=None):
    ...
    info['teacher_compute'] = evidence.get('teacher_compute', {})
    return model, q, info
...
def load_training_assets(cfg, root=ROOT, device='cpu', deadline=None):
    ...
    for configured, actual in (('teacher_sha256', 'teacher_checkpoint_sha256'),
            ('tau_R', 'tau_R'), ('q_ref', 'q_ref'), ('q_cache_sha256', 'q_cache_sha256')):
        if field.get(configured) != reference.get(actual) or field.get(configured) is None:
            raise ValueError('Configured native reference field differs from loaded bytes: ' + configured)
    for configured, actual in (('teacher_checkpoint', 'teacher_checkpoint'), ('q_cache', 'q_cache_path')):
        if (not field.get(configured) or not reference.get(actual)
                or resolved_path(field[configured], root).resolve() != Path(reference[actual]).resolve()):
            raise ValueError('Configured native reference path differs from loaded bytes: ' + configured)
