...
from gfp40.native_data import bind_lane_data, native_method_signature


def fixture(root, schema='G20_DATA_v1'):
    root = Path(root)
...
class NativeLaneDataTests(unittest.TestCase):
    def test_native_wrapper_preserves_original_and_accepted_by_g20_builder(self):
        with tempfile.TemporaryDirectory() as folder:
            original, catalog = fixture(folder)
            before = deepcopy(original)
            with patch('gfp40.native_data.default_sensor_spec', return_value=catalog) as bound:
            ...
            self.assertEqual(provenance['original_manifest_sha256'], provenance['lane_manifest_sha256'])
    ...
    def test_g20_kept_identical_including_extra_metadata(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            with patch('gfp40.native_data.default_sensor_spec', return_value=catalog):
    ...
    def test_changed_lp_or_wrong_catalog_fails_closed(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            with patch('gfp40.native_data.default_sensor_spec', return_value=wrong):
