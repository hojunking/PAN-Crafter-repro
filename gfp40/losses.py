"""P40's registered coefficients; the existing fitting equation is unchanged."""
...
TARGETS = {arm: (1., .1, .002) for arm in ('NATIVE0', 'CTRL', 'MIX')}
...
def coefficients(profile, completed_updates):
    if profile not in TARGETS or int(completed_updates) != completed_updates or completed_updates < 0:
        raise ValueError('Bound P40 profile and nonnegative completed-update index required')
    alpha, beta, edge = TARGETS[profile]
    return dict(alpha=alpha, beta=beta, lambda_edge=edge, ramp_fraction=1.)


def student_losses(out, teacher_out, gt, tau_R, q_weights, profile='NATIVE0',
    ...
    if bands != 4 or y.ndim != 4 or y.shape[1] != 4 or y.shape != teacher.shape or y.shape != target.shape:
        raise ValueError('P40 requires matching C4 Student/Teacher/GT')
