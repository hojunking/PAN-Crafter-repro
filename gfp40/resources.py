...
from gfp40.plan import grid_steps,diagnostic_steps


def idle_evidence():
    processes=[]
    markers=('gfb20_runner.py','g20_runner.py','qg40_runner.py','l100_runner.py','ablr2_runner.py','fh12_runner.py',
    ...
    for path in Path('/proc').glob('[0-9]*/cmdline'):
        try:
            ...
            command=' '.join(argv)
            own=any(Path(v).name=='gfp40_runner.py' and i+1<len(argv) and argv[i+1] in
                    ('preflight','train','family','postrun','replay') for i,v in enumerate(argv))
...
def assess_block(root,cases,*,needs_mixed=False, gamma_count=3):
    ...
    if needs_mixed: estimate+=19809*gamma_count*(64**2+16**2)*4+gamma_count*3072*64**2*4+1024**3
