...
def prepare_data(root, server, spec, deadline_utc=None):
    ...
    if server not in {'s1', 's2', 's3', 's4', 's5'}:
        raise ValueError('Unknown QG40 server')
    if spec.sensor != ('QB' if server in {'s1', 's2', 's4'} else 'GF2'):
    ...
    assets = root / 'work_dir/_qg40' / server / spec.sensor
    for split, binding in bindings.items():
    ...
    import cv2
    manifest = dict(schema='QG40_DATA_v1', server=server, sensor=spec.sensor,
...
def build_dataset(manifest, split, root=None, server=None, augment=False):
    ...
    if manifest.get('schema') != 'QG40_DATA_v1' or manifest.get('num_bands') != 4:
        raise ValueError('Explicit QG40 C4 manifest required')
    ...
    dataset = QG40Dataset(source, lp, spec=spec, split=split, augment=augment)
