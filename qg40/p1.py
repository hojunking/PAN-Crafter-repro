...
from qg40.common import atomic_json, read, sha256
...
def _incomplete_receipt(path, context, error, **extra):
    ...
    logging.getLogger(__name__).warning('QG40 P1 incomplete at %s: %s', path, detail)
    try:
        ...
        logging.getLogger(__name__).warning(
            'QG40 P1 receipt unavailable at %s; BASE may continue, no evidence published: %s: %s',
...
def record_training_diagnostics(wd, model, teacher, datasets, *, update, device,
    ...
    from qg40.training import rng_state, restore_rng
