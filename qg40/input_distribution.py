...
from qg40.common import atomic_json, camp, check_deadline, object_sha, read, utcnow
...
def capture_input_distributions(manifest, *, deadline_utc=None, max_samples=128,
    ...
    report = dict(schema='QG40_INPUT_DISTRIBUTIONS_v1', status='MEASURED', complete=True,
...
def record_input_distributions(root, server, manifest, *, deadline_utc=None, budget_seconds=120, **kwargs):
    ...
    try:
        ...
        report = dict(schema='QG40_INPUT_DISTRIBUTIONS_v1', status='INCOMPLETE', complete=False,
    ...
    if not report.get('complete'):
        logging.getLogger(__name__).warning('QG40 P1 input distribution incomplete: %s', report.get('reason'))
    phase = report.get('qb_phase_residuals', {})
    if phase.get('status') == 'INCOMPLETE':
        logging.getLogger(__name__).warning('QG40 P1 QB phase residual analysis incomplete: %s', phase.get('reason'))
    try:
        atomic_json(path, report)
    except Exception as error:
        report['publication_error'] = str(error)
        logging.getLogger(__name__).warning('QG40 P1 input distribution receipt unavailable at %s: %s', path, error)
    return report
