...
def verify_q_cache(model, dataset, q, *, reference_identity, data_identity,
    ...
    if (not dataset.has_gt or len(dataset) != count or getattr(model, 'bands', None) != 4
            or np.asarray(q).shape != (count, 4) or not np.isfinite(q).all()
            or (np.asarray(q) < 0).any() or (not synthetic_test and count < 3072)):
        raise ValueError('Invalid C4 base-train q-cache parity inputs')
    ...
    check_deadline(deadline)
    return dict(schema='QG40_QCACHE_ONLINE_PARITY_v1', status='MEASURED', passed=True,
...
def validate_receipt(receipt, *, reference_identity, data_identity, q):
    ...
    expected = dict(schema='QG40_QCACHE_ONLINE_PARITY_v1', status='MEASURED', passed=True,
